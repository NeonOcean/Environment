...
from sims4.tuning.merged_tuning_manager import MergedTuningAttr, UnavailablePackSafeResourceError, get_manager
from sims4.tuning.tunable_base import Attributes, BoolWrapper, LoadingAttributes, LoadingTags, MalformedTuningSchemaError, RESERVED_KWARGS, Tags, TunableBase, TunableTypeNotSupportedError, get_default_display_name, tunable_type_mapping
	...
	def create_auto_factory (cls, factory_type, auto_factory_type_name = None, **extra_kwargs):
		...
		class auto_factory(cls):
			...
			def __init__ (self, *args, **kwargs):
				...
				factory_tunables = {
					'description': self.FACTORY_TYPE.__doc__
				}
				is_auto_init = False
				callable_tunables = { }
...
class TunableVariant(TunableTuple):
	...
	VARIANTDEFAULT_LOCKED_ARGS = frozendict({
		VARIANTDEFAULTNONE: None
	})
...
class OptionalTunable(TunableVariant):
	...
	def __init__ (self, tunable, enabled_by_default = False, disabled_value = None, disabled_name = 'disabled', enabled_name = 'enabled', **kwargs):
		...
		super().__init__(locked_args = {
			disabled_name: disabled_value
	...
	def load_etree_node (self, node = None, source = None, **kwargs):
		...
		for child_node in node:
			...
			try:
				...
				if self.allow_none or value is None:
	...
	def __init__ (self, key_type = str, value_type = str, key_value_type = None, key_name = 'key', value_name = 'value', tuple_name = None, **kwargs):
		if key_value_type is not None:
			...
			def key_value_type (**kwargs):
				tuple_def = {
					value_name: _to_tunable(value_type),
					key_name: _to_tunable(key_type)
				}
