...
from sims4.tuning.tunable import Tunable, TunableFactory, TunableList, TunableVariant, get_default_display_name
...
class TunableLocalizedStringFactoryVariant(TunableVariant):
	...
	def __init__ (self, description = 'A localization string. This may either be a single string, or a set to pick a random string from.', allow_none = False, **kwargs):
		super().__init__(single = TunableLocalizedStringFactory(allow_none = allow_none) if self.is_factory else TunableLocalizedString(allow_none = allow_none), variation = self.TunableLocalizedStringFactoryVariation(allow_none = allow_none) if self.is_factory else self.TunableLocalizedStringVariation(allow_none = allow_none), default = 'single', description = description, **kwargs)
