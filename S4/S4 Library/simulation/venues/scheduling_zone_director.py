...
class SchedulingZoneDirectorMixin:
    ...
    def can_schedule_situation(self, situation, **kwargs):
    ...
    def _shift_change_alarm_callback(self, alarm_handle=None):
        ...
        if number_sims_desired_for_shift > 0:
            ...
            for situation_source in situation_shift.additional_sources:
                additional_situations.extend(situation_source.get_additional_situations(predicate=self.can_schedule_situation))
            for _ in range(number_sims_desired_for_shift):
                (situation_to_start, params) = situation_shift.shift_curve.get_situation_and_params(predicate=self.can_schedule_situation, additional_situations=additional_situations)
    ...
    def _handle_situation_shift_churn(self, situation_shift, reserve_object_relationships=False):
        ...
        if number_situations_desire > len(situation_ids):
            reserved_object_relationships = 0
            if reserve_object_relationships:
                reserved_object_relationships = len(situation_ids)
            predicate = lambda situation: self.can_schedule_situation(situation, reserved_object_relationships=reserved_object_relationships)
            additional_situations = []
            for situation_source in situation_shift.additional_sources:
