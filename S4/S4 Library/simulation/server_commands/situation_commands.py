...
SITUATION_ID_BATCH_REPEATED_FIELDS = ('situation_resource_id', 'situation_name', 'category_id', 'mtx_id', 'highest_medal_earned', 'tooltip', 'allow_user_facing_goals')
...
def get_situation_ids(session_id:int=0, sim_id:OptionalTargetParam=None, opt_target_id:int=None, *situation_ids, _connection=None):
    ...
    for (instance, mtx_id, tooltip) in valid_situations:
        with SituationIdEntry.get_entry_with_rollback(situation_batch_msg) as entry:
            ...
            entry.mtx_id = mtx_id
            entry.highest_medal_earned = household.get_highest_medal_for_situation(instance.guid64)
