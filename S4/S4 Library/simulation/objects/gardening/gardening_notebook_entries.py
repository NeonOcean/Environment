...
class NotebookEntryGardeningPlant(NotebookEntry):
    ...
    def get_definition_notebook_data(self, ingredient_cache=()):
        ...
        fruit_definition = definition_manager.get(self.recipe_object_definition_id)
        ...
        sub_list_data = []
        sub_list_data.append(SubListData(None, 0, 0, True, self.entry_text_value(fruit_definition.price/5)))
        ...
        if season_service is not None:
            ...
            sub_list_data.append(SubListData(None, 0, 0, True, season_text))
        sub_list_data.append(SubListData(None, 0, 0, True, LocalizationHelperTuning.get_object_description(plant_definition)))
        ...
        for obj_data in itertools.chain(gardening_collection_data.object_list, gardening_collection_data.bonus_object_list):
            if obj_data.collectable_item is fruit_definition:
                ...
                break
        entry_data = EntryData(LocalizationHelperTuning.get_object_name(fruit_definition), IconInfoData(obj_def_id=fruit_definition.id), None, sub_list_data)
        return entry_data

    def has_identical_entries(self, entries):
        if any(entry.recipe_object_definition_id == self.recipe_object_definition_id for entry in entries):
            return True
        return False
