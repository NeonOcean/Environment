...
from sims4.tuning.tunable import AutoFactoryInit, HasTunableSingletonFactory, OptionalTunable, Tunable, TunableEnumEntry, TunableEnumFlags, TunableEnumWithFilter, TunableFactory, TunableInterval, TunableList, TunableMapping, TunableOperator, TunablePackSafeReference, TunableReference, TunableResourceKey, TunableSet, TunableThreshold, TunableTuple, TunableVariant
...
class BasicStateCheckFactory(TunableFactory):
	...
	def __init__ (self, **kwargs):
		super().__init__(description = "\n            This option tests crafted item's tags against a list of possible\n            tags.", state_set = TunableSet(TunableReference(description = '\n                What state to test for.', manager = services.get_instance_manager(sims4.resources.Types.OBJECT_STATE))), test_type = TunableEnumEntry(description = '\n                How to test the states in the state set against the objects in\n                the inventory.', tunable_type = StateTestType, default = StateTestType.CONTAINS_ANY_STATE_IN_SET), object_requirement = TunableVariant(tag = TunableTuple(tag = CraftTaggedItemFactory(description = '\n                        The object must have this tag.\n                        '), locked_args = {
			'type': BasicStateCheckFactory.TAG_TYPE
		}), definition = TunableTuple(definition = TunableReference(description = '\n                        The object must have this definition.\n                        ', manager = services.definition_manager()), locked_args = {
			'type': BasicStateCheckFactory.DEFINITION_TYPE
		}), locked_args = {
			'any_object': None
...
class ObjectPurchasedTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
	...
	FACTORY_TUNABLES = {
		'test_type': TunableVariant(description = '\n            The object we want to test for.\n            ', object = ObjectTypeFactory.TunableFactory(), tag_set = ObjectTagFactory.TunableFactory(), locked_args = {
			'any_object': None
		}, default = 'any_object'),
	...
	def get_expected_args (self):
		return {
			'obj': event_testing.test_constants.FROM_EVENT_DATA
		}
...
class ObjectEnvironmentScoreTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
	FACTORY_TUNABLES = {
		...
		'environment_score_type': TunableVariant(description = '\n            The type of environment score to test against. This can be mood\n            based, positive scoring, or negative scoring.\n            ', mood_scoring = TunableTuple(description = "\n                Test for a particular mood's environment score on the object.\n                ", mood_to_check = statistics.mood.Mood.TunableReference(description = "\n                    The mood to check the participant's environment scoring.\n                    "), threshold = TunableThreshold(description = "\n                    The threshold for this mood's scoring to pass.\n                    "), locked_args = {
			'scoring_type': EnvironmentScoreType.MOOD_SCORING
			...
			'scoring_type': EnvironmentScoreType.POSITIVE_SCORING
			...
			'scoring_type': EnvironmentScoreType.NEGATIVE_SCORING
	...
	def get_expected_args (self):
		expected_args = {
			'objects_to_test': self.object_to_test
		}
...
class CraftedItemTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
	...
	FACTORY_TUNABLES = {
		'skill_or_item': TunableVariant(description = '\n            Whether to test for a specific item or use of a skill for the item.\n            ', crafted_with_skill = CraftedWithSkillFactory(), crafted_actual_item = CraftActualItemFactory(), crafted_tagged_item = CraftTaggedItemFactory(), default = 'crafted_with_skill'),
	...
	def get_expected_args (self):
		return {
			'crafted_object': event_testing.test_constants.FROM_EVENT_DATA,
...
class InventoryTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
	...
	FACTORY_TUNABLES = {
		...
		'contents_check': TunableVariant(description = '\n            Checks to run on each object of the specified inventory\n            ', has_object_with_tag = CraftTaggedItemFactory(locked_args = {
			...
			'content_check_type': ITEM_DEFINITION_TEST
			...
			'content_check_type': PARTICIPANT_TYPE_TEST
		}), has_object_with_states = BasicStateCheckFactory(locked_args = {
			'content_check_type': ITEM_STATE_TEST
		}), locked_args = {
			'has_anything': None
...
class CustomNameTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
	FACTORY_TUNABLES = {
		'participant': TunableEnumEntry(description = '\n            The subject who is the object of this test.', tunable_type = ParticipantType, default = ParticipantType.Object),
		'has_custom_name': OptionalTunable(Tunable(description = '\n            If checked, the subject must have a custom name set. If unchecked, it cannot have a custom name set.', tunable_type = bool, default = True)),
...
class InUseTest(AutoFactoryInit, HasTunableSingletonFactory, event_testing.test_base.BaseTest):
	...
	FACTORY_TUNABLES = {
		'targets': TunableEnumFlags(description = '\n            Targets to check whether in use.\n            ', enum_type = ParticipantType, default = ParticipantType.Object),
...
class CanSeeObjectTest(AutoFactoryInit, HasTunableSingletonFactory, event_testing.test_base.BaseTest):
	...
	def get_expected_args (self):
		return {
			'sim_info_list': ParticipantType.Actor,
...
class ObjectOwnershipTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
	...
	FACTORY_TUNABLES = {
		'sim': TunableEnumEntry(description = '\n            Presumed Owner.\n            ', tunable_type = ParticipantType, default = ParticipantType.Actor, invalid_enums = (ParticipantType.Invalid,)),
...
class _RadiusFactory(HasTunableSingletonFactory, AutoFactoryInit):
	...
	def get_expected_args (self):
		return {
			'positional_relationship_participants': self.radius_actor
		}
...
class _TaggedObjectOnSameLevelFactory(HasTunableSingletonFactory, AutoFactoryInit):
	...
	def get_expected_args (self):
		return {
			'positional_relationship_participants': self.subject
		}
...
class ObjectCriteriaTest(AutoFactoryInit, HasTunableSingletonFactory, event_testing.test_base.BaseTest):
	...
	FACTORY_TUNABLES = {
		...
		'value': OptionalTunable(description = '\n            A threshold test for the monetary value of a single object in order for it\n            to be considered.\n            ', tunable = TunableTuple(threshold = TunableThreshold(), value_to_check = TunableVariant(locked_args = {
			'catalog_value': lambda x: x.catalog_value,
...
class SituationObjectComparisonTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
	FACTORY_TUNABLES = {
		'identity_test': TunableVariant(description = '\n            Which test to run on the object in order to determine \n            if it matches or not.\n            ', definition_id = ObjectTypeFactory.TunableFactory(), tags = ObjectTagFactory.TunableFactory(), locked_args = {
			'no_identity_test': None
		}, default = 'no_identity_test'),
		'owned': Tunable(description = "\n            If checked will test if the object is owned by the active \n            household. If unchecked it doesn't matter who owns the object or\n            if it is owned at all.\n            ", tunable_type = bool, default = True),
		'on_active_lot': Tunable(description = "\n            If checked, test whether or not the object is on the active\n            lot. If unchecked the object can be either on the active lot or\n            in the open streets area, we don't really care.\n            ", tunable_type = bool, default = False),
