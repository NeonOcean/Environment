...
class StatisticModifierList(HasTunableFactory, AutoFactoryInit):
    ...
    def start(self):
        ...
        if self.lot_statistic_modifiers:
...
class StateComponent(Component, component_name=STATE_COMPONENT, persistence_key=protocols.PersistenceMaster.PersistableData.StateComponent, persistence_priority=ComponentPriority.PRIORITY_STATE, allow_dynamic=True):
    ...
    @componentmethod
    def set_state(self, state, new_value, from_stat=False, from_init=False, from_load=False, immediate=False, force_update=False, **kwargs):
        if not self._check_allowances(new_value, from_load=from_load):
            return
            ...
            if self.owner.parts:
                for part in self.owner.parts:
                    ...
                    if old_value != new_value:
                        if old_value in part.disabling_states:
