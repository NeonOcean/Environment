...
from element_utils import CleanupType, build_critical_section_with_finally, build_element
...
from objects import PaintingState, TunableGeometryState, TunableMaterialState, TunableMaterialVariant, TunableModelSuiteStateIndex, TunableVisibilityState
from objects.client_object_mixin import ClientObjectMixin
from objects.collection_manager import ObjectCollectionData
from objects.components import Component, ComponentPriority, componentmethod, componentmethod_with_fallback
from objects.components.needs_state_value import NeedsStateValue
from objects.components.state_change import StateChange
from objects.components.types import CANVAS_COMPONENT, FLOWING_PUDDLE_COMPONENT, FOCUS_COMPONENT, FOOTPRINT_COMPONENT, LIGHTING_COMPONENT, MANNEQUIN_COMPONENT, STATE_COMPONENT, VIDEO_COMPONENT
...
from sims4.tuning.tunable import AutoFactoryInit, HasTunableFactory, HasTunableReference, HasTunableSingletonFactory, OptionalTunable, Tunable, TunableColor, TunableEnumEntry, TunableEnumFlags, TunableFactory, TunableInterval, TunableList, TunableMapping, TunablePackSafeReference, TunablePercent, TunableRange, TunableReference, TunableResourceKey, TunableSet, TunableSimMinute, TunableTuple, TunableVariant
from sims4.tuning.tunable_hash import TunableStringHash32
from sims4.utils import Result, classproperty, strformatter
...
from statistics.statistic_ops import ObjectStatisticChangeOp, StatisticOperation, TunableStatisticChange
...
class UiMetadataList(HasTunableFactory, AutoFactoryInit, NeedsStateValue):
	FACTORY_TUNABLES = {
		'data': TunableMapping(description = '\n        ', key_type = str, value_type = TunableVariant(other_value = TunableVariant(default = 'integral', boolean = Tunable(bool, False), string = TunableLocalizedString(), integral = Tunable(int, 0), icon = TunableIcon(), color = TunableColor()), state_value = TunableVariant(default = 'value', locked_args = {
...
class ObjectValueChangeOperation(HasTunableFactory, AutoFactoryInit):
	FACTORY_TUNABLES = {
		'value_change_type': TunableVariant(decrease_value = ValueDecreaseFactory.TunableFactory(), increase_value = ValueIncreaseFactory.TunableFactory(), default = 'decrease_value'),
...
class StateChangeOperation(HasTunableSingletonFactory):
	FACTORY_TUNABLES = {
		'tint': OptionalTunableClientStateChangeItem(tunable = TunableColorSnippet(description = 'A tint to apply')),
		...
		StateChange.VFX: OptionalTunableClientStateChangeItemWithDisable(description = '\n            Play one or more visual effects.\n            ', tunable = TunableVariant(description = '\n                Define the type of visual effect to play.\n                ', single_effect = PlayEffect.TunableFactory(), multiple_effects = PlayMultipleEffects.TunableFactory(), default = 'single_effect'), tunable_disabled_name = 'no_vfx', tunable_enabled_name = 'start_vfx'),
		...
		StateChange.GLOW: OptionalTunableClientStateChangeItemWithDisable(tunable = Glow.TunableFactory(), tunable_disabled_name = 'no_glow', tunable_enabled_name = 'start_glow'),
		StateChange.AUDIO_STATE: OptionalTunableClientStateChangeItemWithDisable(tunable = TunablePlayAudio(description = 'An audio state to apply'), tunable_disabled_name = 'no_audio', tunable_enabled_name = 'start_audio'),
		StateChange.AUDIO_EFFECT_STATE: OptionalTunableClientStateChangeItemWithDisable(description = '\n            A way to apply An audio effect (.effectx) to the object when state changes\n            ', tunable = ApplyAudioEffect.TunableFactory(), tunable_disabled_name = 'no_audio_effect', tunable_enabled_name = 'start_audio_effect'),
		StateChange.VIDEO_STATE: OptionalTunableClientStateChangeItem(
			tunable = OptionalTunable(disabled_name = 'no_video', enabled_name = 'start_video', tunable = TunableTuple(description = '\n                    List of clip names and append behavior.\n                    ', clip_list = TunableList(TunableResourceKey(None, resource_types = [RESOURCE_TYPE_VP6])), append_clip = Tunable(description = '\n                        If enabled clip list will be appended to previous \n                        playing clip instead of interrupting the existing \n                        playlist.\n                        This should be tuned when the clips should have a \n                        smoother transition like a credits scene on a movie.\n                        ', tunable_type = bool, default = False), loop_last = Tunable(description = "\n                        If enabled, the clip will loop. Otherwise, it's played\n                        once and stop on last frame.\n                        ", tunable_type = bool, default = True)))),
		StateChange.VIDEO_STATE_LOOPING: OptionalTunableClientStateChangeItemWithDisable(tunable = TunableResourceKey(None, resource_types = [sims4.resources.Types.PLAYLIST]), tunable_disabled_name = 'no_video', tunable_enabled_name = 'start_video'),
		...
		StateChange.AUTONOMY_MODIFIERS: OptionalTunableClientStateChangeItemWithDisable(tunable = StatisticModifierList.TunableFactory(), tunable_disabled_name = 'no_statistic_to_apply', tunable_enabled_name = 'apply_statistic_modifiers'),
		StateChange.AWARENESS: OptionalTunableClientStateChangeItemWithDisable(tunable = AwarenessSourceRequest.TunableFactory(), tunable_disabled_name = 'no_awareness', tunable_enabled_name = 'start_awareness'),
		StateChange.BROADCASTER: OptionalTunableClientStateChangeItem(tunable = OptionalTunable(tunable = BroadcasterRequest.TunableFactory(locked_args = {
			'participant': None,
			'offset_time': None
		}), disabled_name = 'no_broadcaster', enabled_name = 'start_broadcaster')),
		StateChange.ROUTE_EVENT: OptionalTunableClientStateChangeItem(tunable = OptionalTunable(tunable = RouteEventProviderRequest.TunableFactory(locked_args = {
			'participant': None
		}))),
		StateChange.WALKSTYLE: OptionalTunableClientStateChangeItem(tunable = OptionalTunable(tunable = WalkStyleRequest.TunableFactory(), disabled_name = 'no_request', enabled_name = 'request_walkstyle')),
		StateChange.REPLACE_OBJECT: OptionalTunable(disabled_value = UNSET, tunable = ObjectReplacementOperation.TunableFactory()),
		...
		StateChange.CHANGE_VALUE: OptionalTunable(disabled_value = UNSET, tunable = ObjectValueChangeOperation.TunableFactory()),
	...
	USE_COMPONENT_FOR = {
		StateChange.PAINTING_REVEAL_LEVEL: CANVAS_COMPONENT.instance_attr,
...
class ObjectStateValue(HasTunableReference, ObjectStateValueDisplayMixin, SuperAffordanceProviderMixin, metaclass = ObjectStateMetaclass, manager = get_instance_manager(sims4.resources.Types.OBJECT_STATE)):
	INSTANCE_TUNABLES = {
		'value': TunableVariant(locked_args = {
			'unordered': None
		}, boolean = Tunable(bool, True), integral = Tunable(int, 0), decimal = Tunable(float, 0)),
...
class ChannelBasedObjectStateValue(ObjectStateValue):
	...
	INSTANCE_TUNABLES = {
		'show_in_picker': Tunable(bool, True, description = 'If True than this channel will not be displayed to be chosen in the channel picker dialog.')
	}
...
class VideoChannel(ChannelBasedObjectStateValue):
	INSTANCE_TUNABLES = {
		'affordance': TunableReference(manager = services.get_instance_manager(sims4.resources.Types.INTERACTION), allow_none = True)
	}
...
class ObjectState(HasTunableReference, ObjectStateValueDisplayMixin, metaclass = ObjectStateMetaclass, manager = get_instance_manager(sims4.resources.Types.OBJECT_STATE)):
	INSTANCE_TUNABLES = {
		'overridden_by': OptionalTunable(TunableReference(manager = get_instance_manager(sims4.resources.Types.OBJECT_STATE), class_restrictions = 'ObjectState')),
...
class CommodityBasedObjectState(ObjectState):
	INSTANCE_TUNABLES = {
		'linked_stat': TunableReference(description = '\n            The statistic to link to the state.\n            ', manager = get_instance_manager(sims4.resources.Types.STATISTIC)),
		'_values': TunableList(tunable = TunableStateValueReference(class_restrictions = CommodityBasedObjectStateValue), unique_entries = True, minlength = 1),
...
class StateComponent(Component, component_name = STATE_COMPONENT, persistence_key = protocols.PersistenceMaster.PersistableData.StateComponent, persistence_priority = ComponentPriority.PRIORITY_STATE, allow_dynamic = True):
	...
	def on_remove (self, *_, **__):
		if not (self.owner.is_sim or not services.current_zone().is_zone_shutting_down):
			self._cleanup_client_state()
	...
	def set_state (self, state, new_value, from_stat = False, from_init = False, immediate = False, force_update = False):
		...
		if not from_init:
			if new_value == old_value and not force_update and not value_force_added:
				return
			if not from_stat:
	...
	def _add_stat_listener (self, state, new_value):
		...
		if stat_type in self._commodity_states:
			...
			if not tracker.has_statistic(stat_type) and stat_type.added_by_default():
...
class TunableStateTriggerTestVariant(TunableVariant):
	...
	def __init__ (self, description = 'A single tunable test.', **kwargs):
		super().__init__(object_criteria = objects.object_tests.ObjectCriteriaTest.TunableFactory(locked_args = {
			'tooltip': None
		}), festival_running = FestivalRunningTest.TunableFactory(locked_args = {
			'tooltip': None
		}), next_festival = NextFestivalTest.TunableFactory(locked_args = {
			'tooltip': None
...
class StateTrigger(HasTunableSingletonFactory, AutoFactoryInit):
	...
	FACTORY_TUNABLES = {
		...
		}, statistic_override = StatisticOperation.get_statistic_override(pack_safe = True), include_relationship_ops = False)),
...
class TunableStateChange(TunableFactory):
	...
	class ValueFromTestList(HasTunableSingletonFactory):
		...
		def FACTORY_TUNABLES (cls):
			...
			return {
				'new_values': TunableList(TunableTuple(test = TunableTestVariant(test_locked_args = {
					'tooltip': None
	...
	class ValueFromTestSetList(ValueFromTestList):
		...
		def FACTORY_TUNABLES (cls):
			from event_testing.tests import TunableTestSet
			return {
				'new_values': TunableList(TunableTuple(tests = TunableTestSet(), value = TunableStateValueReference()))
			}
...
class TunableTransienceChange(TunableFactory):
	...
	def __init__ (self, description = "Change the transience on the interaction's target.", **kwargs):
		super().__init__(who = TunableEnumEntry(ParticipantType, ParticipantType.Object, description = 'Who or what to apply this test to'), new_value_beginning = TunableVariant(locked_args = {
			'no_change': None,
			'make_transient': True,
			'make_permanent': False
		}, default = 'no_change', description = 'A value to set transience to at the beginning (may be None)'), new_value_ending = TunableVariant(locked_args = {
			'no_change': None,
			'make_transient': True,
			'make_permanent': False
		}, default = 'no_change', description = 'A value to set transience to at the beginning (may be None)'), xevt_id = OptionalTunable(Tunable(int, 100, description = "An xevt on which to change the state's value to new_value_ending")), criticality = TunableEnumEntry(CleanupType, CleanupType.NotCritical, description = 'The criticality of making these state changes.'), description = description, **kwargs)
