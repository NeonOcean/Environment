...
from objects import TunableVisibilityState, TunableGeometryState, TunableMaterialState, TunableMaterialVariant, PaintingState
    ...
    def pre_add(self, *_, **__):
        for (state, value) in self.items():
            if state.linked_stat is not None:
                ...
                linked_stat = tracker.get_statistic(state.linked_stat, add=True)
                if linked_stat.use_stat_value_on_initialization:
        ...
        if stat_type in self._commodity_states:
            ...
            def add_listener(preferred_value, threshold):
                ...
                if listener is not None:
                    ...
                    logger.error('Failed to create listener for: {}', stat_type.__name__, owner='tingyul')
...
def state_change(targets, new_value_beginning=None, new_value_ending=None, xevt_id=None, animation_context=None, criticality=CleanupType.OnCancel, sequence=()):
    queue = []
    if not targets:
        return sequence
    ...
    if set_at_beginning:
        ...
        def set_beginning_target_refs_states(*_, **__):
            for resolved_target in target_refs:
                resolved_target.set_state(new_value_beginning.state, new_value_beginning)
...
class TunableStateChange(TunableFactory):
    ...
    def _factory(interaction, state_change_target, timing, new_value, **kwargs):
        ...
        return timing(new_value=actual_new_value, targets=actual_state_change_targets, animation_context=interaction.animation_context, **kwargs)
