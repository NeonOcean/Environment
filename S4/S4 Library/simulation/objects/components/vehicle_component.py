...
from interactions.constraints import Circle, Constraint, ANYWHERE
...
from objects.vehicle_controls import VehicleControlWheel
from postures.posture_specs import PostureSpecVariable
from postures.posture_state_spec import PostureStateSpec
from protocolbuffers import Vehicles_pb2, DistributorOps_pb2
from routing import SurfaceType
from sims4.tuning.tunable import AutoFactoryInit, HasTunableFactory, Tunable, TunableEnumSet, TunableReference, TunableTuple, OptionalTunable, TunableList, TunableVariant, TunableRange, TunedInterval
...
class VehicleComponent(FootprintToggleMixin, Component, HasTunableFactory, AutoFactoryInit, component_name=types.VEHICLE_COMPONENT):
    ...
    def send_vehicle_data_msg(self):
        vehicle_data_msg = Vehicles_pb2.VehicleData()
        for (control_id, control) in self._vehicle_controls.items():
            with ProtocolBufferRollback(vehicle_data_msg.controls) as control_msg:
                control_msg.control_id = control_id
                control.build_control_msg(control_msg)
        distributor = Distributor.instance()
        op = GenericProtocolBufferOp(DistributorOps_pb2.Operation.SET_VEHICLE_DATA, vehicle_data_msg)
