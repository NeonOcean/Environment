...
class Zone:
    ...
    def _get_spawn_points_with_lot_id_and_tags(self, sim_info=None, lot_id=None, sim_spawner_tags=None, except_lot_id=None, spawn_point_request_reason=SpawnPointRequestReason.DEFAULT):
        ...
        for spawn_point in self.spawn_points_gen():
            if lot_id is not None and spawn_point.lot_id != lot_id:
                continue
            if except_lot_id is not None and spawn_point.lot_id == except_lot_id:
                continue
    ...
    def validate_spawn_points(self):
        if not self._world_spawn_points and not self._dynamic_spawn_points:
            return
        dest_handles = set()
        lot_center = self.lot.center
    ...
    def update_household_objects_ownership(self):
        ...
        if self.lot.owner_household_id == 0:
            travel_group = self.travel_group_manager.get_travel_group_by_zone_id(self.id)
            if travel_group is None or not protocol_buffer_utils.has_field(gameplay_zone_data, 'active_travel_group_id_on_save') or gameplay_zone_data.active_travel_group_id_on_save != travel_group.id:
                self._set_zone_objects_household_owner_id(None)
        elif self.lot.owner_household_id == services.active_household_id() and (not protocol_buffer_utils.has_field(gameplay_zone_data, 'active_household_id_on_save') or gameplay_zone_data.lot_owner_household_id_on_save != services.active_household_id()):
