...
from protocolbuffers.Consts_pb2 import MGR_OBJECT, MGR_PARTY, MGR_SITUATION, MGR_SOCIAL_GROUP, MGR_TRAVEL_GROUP
...
from world.spawn_point import SpawnPoint, SpawnPointOption
...
class Zone:
	...
	def start_services (self, gameplay_zone_data, save_slot_data):
		...
		service_list = [SimIrqService(), PlexService(), ResetAndDeleteService(), ObjectManager(manager_id = MGR_OBJECT), InventoryManager(manager_id = MGR_OBJECT), ConditionalLayerService(), DoorService(), PropManager(manager_id = MGR_OBJECT), TravelGroupManager(manager_id = MGR_TRAVEL_GROUP), PostureGraphService(), ArbAccumulatorService(None, None), AutonomyService(), SimSpawnerService(), EnsembleService(), SituationManager(manager_id = MGR_SITUATION), DemographicsService(), SimFilterService(), NPCHostedSituationService(), LandlordService(), PartyManager(manager_id = MGR_PARTY), SocialGroupManager(manager_id = MGR_SOCIAL_GROUP), UiDialogService(), CalendarService(), DramaScheduleService(), ObjectClusterService(), SocialGroupClusterService(), NeighborhoodPopulationService(), ServiceNpcService(), LotSpawnerService(), VenueService(), AmbientService(), StoryProgressionService(), ZoneSpinUpService(), PrivacyService(), FireService(), ZoneModifierService(), BroadcasterService(),
						BroadcasterRealTimeService(), CleanupService(), CareerService(), DaycareService(), LaundryService(), PhotographyService(), AdoptionService(), CullingService(), GardeningService(), MasterController()]
