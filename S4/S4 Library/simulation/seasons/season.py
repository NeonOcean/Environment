...
from sims4.tuning.tunable import AutoFactoryInit, HasTunableFactory, HasTunableReference, HasTunableSingletonFactory, OptionalTunable, TunableEnumEntry, TunableList, TunableMapping, TunableRange, TunableReference, TunableTuple
from sims4.tuning.tunable_base import ExportModes, GroupNames
from tunable_time import Days, TunableTimeOfDay, TunableTimeSpan
from ui.screen_slam import TunableScreenSlamSnippet

class DayOfSeason(HasTunableFactory, AutoFactoryInit):
	FACTORY_TUNABLES = {
		'week_of_season': TunableRange(description = '\n            Which week of the season this is.  First week is week 0.\n            ', tunable_type = int, default = 0, minimum = 0, maximum = 7),
...
class SeasonalContent(HasTunableSingletonFactory, AutoFactoryInit):
	FACTORY_TUNABLES = {
		'holidays': TunableMapping(key_type = TunableReference(description = '\n                Drama node to be scheduled for this holiday.\n                ', manager = services.get_instance_manager(sims4.resources.Types.HOLIDAY_DEFINITION), pack_safe = True), value_type = TunableList(tunable = DayOfSeason.TunableFactory())),
		'segments': TunableTuple(
			early_season_length = TunableTimeSpan(description = '\n                Early season length, in days.\n                ', default_days = 2, locked_args = {
				'hours': 0,
				'minutes': 0
