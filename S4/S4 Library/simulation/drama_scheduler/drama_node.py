...
from date_and_time import DATE_AND_TIME_ZERO, DateAndTime, TimeSpan, create_time_span, date_and_time_from_week_time
...
from gsi_handlers.drama_handlers import DramaNodeLogActions, GSIRejectedDramaNodeScoringData, is_drama_node_log_enabled, log_drama_node_scoring
from interactions import ParticipantType, ParticipantTypeSingleSim
from interactions.utils.tunable_icon import TunableIcon
from objects import ALL_HIDDEN_REASONS
from protocolbuffers import Dialog_pb2, UI_pb2
...
from sims4.tuning.tunable import HasTunableReference, OptionalTunable, Tunable, TunableEnumEntry, TunableInterval, TunableList, TunableRange, TunableReference, TunableTuple, TunableVariant
...
class _DramaParticipant(TunableVariant):
	...
	def __init__ (self, description = 'A drama participant', default = 'participant_type', excluded_options = (), **kwargs):
		options = {
			'no_participant': TunableTuple(description = ',\n                There is no participant for this drama participant.\n                ', locked_args = {
				'type': DramaNodeParticipantOption.DRAMA_PARTICIPANT_OPTION_NONE
			}),
			'participant_type': TunableTuple(description = '\n                Use a specific participant type from the system that called\n                this drama node to resolve this drama participant.\n                ', participant_type = TunableEnumEntry(description = "\n                    The participant type to use to resolve the drama node's\n                    participant.\n                    ", tunable_type = ParticipantType, default = ParticipantType.Actor), locked_args = {
				'type': DramaNodeParticipantOption.DRAMA_PARTICIPANT_OPTION_PARTICIPANT_TYPE
			}),
...
class BaseDramaNode(HasTunableReference, metaclass = HashedTunedInstanceMetaclass, manager = services.get_instance_manager(sims4.resources.Types.DRAMA_NODE)):
	...
	def get_final_times_based_on_schedule (self, anchor_time = None, scheduled_time_only = False, time_modifier = TimeSpan.ZERO):
		...
		if not self.allow_during_work_hours and not self.simless:
