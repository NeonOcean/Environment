...
import sims4.log
import tag
from distributor.shared_messages import IconInfoData, build_icon_info_msg, create_icon_info_msg
from distributor.system import Distributor
from interactions.utils.tunable_icon import TunableIconFactory
from objects.slots import SlotType
from protocolbuffers import Consts_pb2, Dialog_pb2, UI_pb2
from sims.outfits.outfit_enums import OutfitCategory, REGULAR_OUTFIT_CATEGORIES
from sims4.localization import LocalizationHelperTuning, NULL_LOCALIZED_STRING_FACTORY, TunableLocalizedString, TunableLocalizedStringFactory, TunableLocalizedStringFactoryVariant
from sims4.math import MAX_INT16
from sims4.tuning.tunable import AutoFactoryInit, HasTunableSingletonFactory, OptionalTunable, Tunable, TunableEnumEntry, TunableEnumFlags, TunableEnumSet, TunableList, TunableRange, TunableReference, TunableResourceKey, TunableTuple, TunableVariant
...
class PickerColumn(HasTunableSingletonFactory, AutoFactoryInit):
	...
	FACTORY_TUNABLES = {
		'column_type': TunableEnumEntry(description = '\n            The type of column.\n            ', tunable_type = ColumnType, default = ColumnType.ICON_AND_TEXT),
		'label': OptionalTunable(description = '\n            If enabled, the text to show on the column. \n            ', tunable = TunableLocalizedString()),
		'icon': OptionalTunable(description = '\n            If enabled, the icon to show on the column.\n            ', tunable = TunableResourceKey(resource_types = sims4.resources.CompoundTypes.IMAGE, default = None)),
...
class UiRecipePicker(UiDialogObjectPicker):
	...
	FACTORY_TUNABLES = {
		'skill': OptionalTunable(Skill.TunableReference(description = '\n            The skill associated with the picker dialog.\n            ')),
...
class UiSimPicker(UiDialogObjectPicker):
	FACTORY_TUNABLES = {
		'column_count': TunableRange(description = '\n            Define the number of columns to display in the picker dialog.\n            ', tunable_type = int, default = 3, minimum = 3, maximum = 8),
...
class UiOutfitPicker(UiDialogObjectPicker):
	...
	FACTORY_TUNABLES = {
		'thumbnail_type': TunableEnumEntry(description = '\n            Define how thumbnails are to be rendered.\n            ', tunable_type = _OutftiPickerThumbnailType, default = _OutftiPickerThumbnailType.SIM_INFO),
		'outfit_categories': TunableEnumSet(description = '\n            The categories to display.\n            ', enum_type = OutfitCategory, default_enum_list = REGULAR_OUTFIT_CATEGORIES),
