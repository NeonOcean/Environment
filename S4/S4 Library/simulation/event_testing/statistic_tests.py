...
from sims4.tuning.tunable import AutoFactoryInit, HasTunableSingletonFactory, OptionalTunable, Tunable, TunableEnumEntry, TunableFactory, TunableList, TunableOperator, TunablePackSafeReference, TunableReference, TunableSet, TunableThreshold, TunableTuple, TunableVariant
...
class SpecifiedStatThresholdMixin:
	...
	def participant_type_override (participant_type_enum, participant_type_default):
		return {
			'who': TunableEnumEntry(participant_type_enum, participant_type_default, description = 'Who or what to apply this test to')
		}
...
class TunableStatOfCategoryTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
	...
	FACTORY_TUNABLES = {
		'who': TunableEnumEntry(description = '\n            Who or what to apply this test to.\n            ', tunable_type = ParticipantType, default = ParticipantType.Actor),
		'statistic_category': TunableEnumEntry(description = '\n            The category to check for.\n            ', tunable_type = statistics.statistic_categories.StatisticCategory, default = statistics.statistic_categories.StatisticCategory.INVALID, pack_safe = True),
...
class CommodityAdvertisedTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
	...
	def get_expected_args (self):
		expected_args = {
			'target_objects': ParticipantType.Object,
...
class StatisticEquivalencyTest(HasTunableSingletonFactory, AutoFactoryInit, event_testing.test_base.BaseTest):
	...
	def get_expected_args (self):
		return {
			'participant_stat': ParticipantType.PickedStatistic
		}

	@cached_test
	def __call__ (self, participant_stat):
		for tested_stat in participant_stat:
			if tested_stat is not None:
				if self.stat is tested_stat:
					return TestResult.TRUE
