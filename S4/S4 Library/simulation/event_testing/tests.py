...
from fame.fame_tests import FameMomentTest, LifestyleBrandTest
...
from holidays.holiday_tests import HolidayTest, TraditionTest
...
from objects.slot_tests import RelatedSlotsTest, TunableSlotTest
...
from sims.household_utilities.utility_tests import UtilitiesComponentTest, UtilityTest
from sims.occult.occult_tests import OccultFormAvailabilityTest
from sims.outfits.outfit_tests import OutfitBodyTypeTest, OutfitTest
from sims.university.university_tests import UniversityClassroomTest, UniversityEnrollmentTest, UniversityHousingConfigurationTest, UniversityProfessorTest, UniversityTests
...
class TunableTestVariant(sims4.tuning.tunable.TunableVariant):
	TEST_VARIANTS = {
		...
		'test_set_reference': lambda **__: sims4.tuning.tunable.TunableReference(manager = services.get_instance_manager(sims4.resources.Types.SNIPPET), class_restrictions = ('TestSetInstance',), pack_safe = True),
		...
		'travel_group': event_testing.test_variants.TravelGroupTest.TunableFactory,
...
class TunableTestSet(_TunableTestSetBase, is_fragment = True):
	...
	def __init__ (self, **kwargs):
		super().__init__(test_locked_args = {
			'tooltip': None
		}, **kwargs)
...
class TestSetInstance(metaclass = sims4.tuning.instances.HashedTunedInstanceMetaclass, manager = services.get_instance_manager(sims4.resources.Types.SNIPPET)):
	INSTANCE_TUNABLES = {
		'test': TunableTestSetWithTooltip()
	}
