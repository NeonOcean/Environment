...
class _UniqueWorldsObjectiveTrack(_ObjectiveCompletionType):
    FACTORY_TUNABLES = {'unique_worlds_required_to_pass': sims4.tuning.tunable.Tunable(description='\n                The number of unique worlds needed to pass.\n                ', tunable_type=int, default=1)}
...
class BaseObjective:
    INSTANCE_TUNABLES = {'display_text': TunableLocalizedStringFactory(description='\n            Text used to show the progress of this objective in the UI\n            ', allow_none=True, export_modes=sims4.tuning.tunable_base.ExportModes.All), 'display_age_list': OptionalTunable(description='\n            If enabled, the Sim must be one of the specified ages to test into the objective.\n            ', tunable=TunableEnumSet(enum_type=Age, enum_default=Age.ADULT, default_enum_list=[Age.TODDLER, Age.CHILD, Age.TEEN, Age.YOUNGADULT, Age.ADULT, Age.ELDER]), enabled_by_default=False, export_class_name='DisplayAgeListOptionalTunable', export_modes=sims4.tuning.tunable_base.ExportModes.All), 'satisfaction_points': sims4.tuning.tunable.Tunable(description='\n            The number of satisfaction points, if relevant, that completion of this objective awards.', tunable_type=int, default=0, export_modes=sims4.tuning.tunable_base.ExportModes.All), 'resettable': sims4.tuning.tunable.Tunable(description='\n            Setting this allows for this objective to reset back to zero for\n            certain uses, such as for Whim Set activation.\n            ', tunable_type=bool, default=False), 'tooltip': TunableLocalizedStringFactory(description='\n            Tooltip that will display on the objective.\n            ', allow_none=True, export_modes=sims4.tuning.tunable_base.ExportModes.All), 'relative_to_unlock_moment': sims4.tuning.tunable.Tunable(description="\n            If true this objective will start counting from the moment of assignment or reset instead\n            of over the total lifetime of a Sim, most useful for Careers and Whimsets. Note:\n            this effect is only for 'Total' data tests (tests that used persisted save data)\n             ", tunable_type=bool, default=False)}
