...
_PostureManifestEntry = namedtuple('_PostureManifestEntry', ('actor', 'specific', 'family', 'level', 'left', 'right', 'surface', 'provides', 'target_object_filter'))
...
class PostureManifestEntry(_PostureManifestEntry, InternMixin):
    ...
    def apply_actor_map(self, actor_map):
        ...
        return self.__class__(actor, self.specific, self.family, self.level, left, right, surface, self.provides, target_object_filter=self.target_object_filter)
    ...
    def carry_target(self):
        if self.left not in _NOT_SPECIFIC_ACTOR:
            return self.left
        elif self.right not in _NOT_SPECIFIC_ACTOR:
            return self.right
    ...
    def free_hands(self) -> Hand:
        if self.left in (MATCH_ANY, HOLSTER):
            ...
            return (Hand.LEFT,)
        elif self.right in (MATCH_ANY, HOLSTER):
            return (Hand.RIGHT,)
        return ()
