...
def generate_aspiration_view_data(sim_id:int=None):
    ...
    for aspiration_id in aspiration_manager.types:
        ...
        if sim_info.aspiration_tracker.milestone_completed(aspiration):
            aspiration_data['aspiration_complete'] = True
        for objective in aspiration.objectives:
            objective_data = {}
            objective_data['objective'] = str(objective)
            objective_data['objective_iterations_complete'] = sim_info.aspiration_tracker.get_last_updated_value_for_objective(objective.guid64)
            objective_data['objective_iterations_required'] = objective.goal_value()
            objective_data['objective_complete'] = sim_info.aspiration_tracker.objective_completed(objective)
