...
class RabbitHoleService(Service):
    ...
    def put_sim_in_managed_rabbithole(self, sim_info, rabbit_hole_type, ignore_exit_conditions=False, picked_skill=None):
        ...
        found_affordance = None
        resolver = SingleSimResolver(sim_info)
        for item in rabbit_hole_type.tested_affordances:
            if item.tests.run_tests(resolver):
                found_affordance = item.affordance
                break
        if sim_info.is_instanced(allow_hidden_flags=ALL_HIDDEN_REASONS):
            affordance_to_use = found_affordance or rabbit_hole_affordance
            if sim_info.is_at_home or found_affordance is not None:
                result = self._push_affordance(sim_info.id, affordance_to_use, picked_statistic=picked_skill)
                if not result:
                    return
                self._register_callback_for_cancel(sim_info, affordance_to_use)
                self._register_for_away_action_at_start_of_interaction(affordance_to_use)
            else:
                result = self._push_affordance(sim_info.id, go_home_affordance, picked_statistic=picked_skill)
            ...
            self._move_sim_home_and_setup(sim_info, rabbit_hole_affordance, away_action, picked_statistic=picked_skill)
        else:
        ...
        self._store_rabbit_hole(sim_info.id, rabbit_hole_type, time, linked_sim_ids, picked_skill)
    ...
    def restore_rabbit_hole_state(self):
        ...
        for (sim_id, rabbit_hole) in self._rabbit_holes.items():
            ...
            if sim_info is None:
                ...
                found_affordance = None
                resolver = SingleSimResolver(sim_info)
                for item in rabbit_hole.tested_affordances:
                    if item.tests.run_tests(resolver):
                        found_affordance = item.affordance
                        break
                picked_skill = rabbit_hole.get_participant(ParticipantType.PickedStatistic)
                if sim_info.is_instanced(allow_hidden_flags=ALL_HIDDEN_REASONS):
                    affordance_to_use = found_affordance or rabbit_hole.affordance
                    result = self._push_affordance(sim_id, affordance_to_use, picked_statistic=picked_skill)
                    if not result:
                        linked_sims_to_free.extend(rabbit_hole.linked_sim_ids)
                    else:
                        self._register_callback_for_cancel(sim_info, affordance_to_use)
                        ...
                        self._attach_exit_conditions(sim_info.id)
                elif found_affordance is not None:

                    def _on_spawn(sim):
                        result = self._push_affordance(sim_id, found_affordance, picked_statistic=picked_skill)
                        if not result:
                            linked_sims_to_free.extend(rabbit_hole.linked_sim_ids)
                            return
                        self._register_callback_for_cancel(sim_info, found_affordance)
                        self._set_up_away_action(sim_info, rabbit_hole.away_action)

                    SimSpawner.spawn_sim(sim_info, spawn_action=_on_spawn)
                elif not sim_info.is_at_home:
                    self._move_sim_home_and_setup(sim_info, rabbit_hole.affordance, rabbit_hole.away_action, picked_statistic=picked_skill)
                else:
                    self._set_up_away_action(sim_info, rabbit_hole.away_action)
                self._add_sim_to_blacklist(sim_id)
                self._attach_exit_conditions(sim_info.id)
    ...
    def save(self, save_slot_data=None, **kwargs):
        ...
        for (sim_id, rabbit_hole) in self._rabbit_holes.items():
            with ProtocolBufferRollback(rabbit_hole_service_proto.rabbit_holes) as entry:
                ...
                picked_stat = rabbit_hole.get_participant(ParticipantType.PickedStatistic)
                if picked_stat is not None:
                    entry.picked_stat_id = picked_stat.guid64
    ...
    def load(self, **_):
        ...
        for entry in save_slot_data.gameplay_data.rabbit_hole_service.rabbit_holes:
            ...
            if entry.HasField('time_remaining'):
                ...
                time = None
            picked_skill = None
            if entry.HasField('picked_stat_id'):
                picked_skill = get_instance_manager(sims4.resources.Types.STATISTIC).get(entry.picked_stat_id)
            self._store_rabbit_hole(sim_id, rabbit_hole_type, time, list(entry.linked_sim_ids), picked_skill)
    ...
    def _push_affordance(self, sim_id, affordance, **kwargs):
        ...
        return sim.push_super_affordance(affordance, sim, context, **kwargs)
    ...
    def _on_sim_moved_home(self, sim_info, rabbit_hole_affordance, away_action, **kwargs):
        push_success = self._push_affordance(sim_info.id, rabbit_hole_affordance, **kwargs)
        if not push_success:
            self._remove_rabbit_hole(sim_info.id, canceled=True)
            return
        self._set_up_away_action(sim_info, away_action)

    def _move_sim_home_and_setup(self, sim_info, rabbit_hole_affordance, away_action, **kwargs):
        if sim_info.is_instanced(allow_hidden_flags=ALL_HIDDEN_REASONS):
            return
        ...
        if services.current_zone_id() != home_zone_id:
            ...
            SimSpawner.spawn_sim(sim_info, spawn_action=lambda _: self._on_sim_moved_home(sim_info, rabbit_hole_affordance, away_action, **kwargs), update_skewer=False)

    def _store_rabbit_hole(self, sim_id, rabbit_hole_type, time, linked_sim_ids, picked_skill):
        if time is not None:
            alarm_handle = self._set_up_expiration_alarm(sim_id, time)
        else:
            alarm_handle = None
        self._rabbit_holes[sim_id] = rabbit_hole_type(sim_id, alarm_handle, CallableList(), linked_sim_ids, picked_skill)
