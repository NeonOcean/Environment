...
class RoommateService(Service):
    ...
    def _process_continuing_leave_reason(self, sim_info, sim_data_reason, leave_reason, time_span):
        ...
        if not sim_data_reason.been_warned and (reason_data.time_till_warning is None or total_time >= reason_data.time_till_warning()):
    ...
    def get_auto_invite_sim_infos(self, host_sim, situation):
        ...
        for sim_id in self._zone_to_roommates[home_zone_id].sim_ids:
            ...
            if sim_info.has_trait(self.AUTO_INVITE_TRAIT):
                if random.random() <= self.AUTO_INVITE_CHANCE:
    ...
    def on_all_households_and_sim_infos_loaded(self, client):
        for roommate_id in self.get_roommate_ids():
            ...
            if sim_info is None or not (sim_info.household.home_zone_id != 0 or not sim_info.can_instantiate_sim):
    ...
    def _do_rent(self, *_):
        ...
        if not unpaid_sim_strings and total_rent == 0:
    ...
    def _reacquire_roommates(self, *_, on_lot_only=False):
        ...
        if zone_id in self._zone_to_roommates:
            ...
            for sim_id in sim_ids:
                ...
                if is_busy:
                    continue
                if on_lot_only and sim_info.zone_id != zone_id:
                    continue
    ...
    def get_available_roommate_count(self, beds=None, remove_extra=False):
        ...
        if zone_id in self._zone_to_roommates:
            ...
            if not remove_extra and max_possible_new < 0:
                return -1
        if beds is None:
            beds = set(services.object_manager().get_objects_with_tags_gen(*self.BED_TAGS))
