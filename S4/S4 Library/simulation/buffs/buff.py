		...
		if self._static_commodites_added:
			for static_commodity_type in self._static_commodites_added:
				...
				if tracker is not None:
					if not tracker.has_statistic(static_commodity_type):
						tracker.add_statistic(static_commodity_type)
		resolver = sim.get_resolver()
		...
		for route_event_cls in self.route_events.single_events:
			if route_event_cls is not None:
				if not failed_types is None:
					if route_event_cls not in failed_types:
						if not route_event_context.route_event_already_scheduled(route_event_cls):
							if not route_event_context.route_event_already_fully_considered(route_event_cls, self):
								if route_event_cls.test(resolver):
