...
from buffs import Appropriateness, BuffPolarity
...
from objects.mixins import MixerActorMixin, MixerProviderMixin, SuperAffordanceProviderMixin, TargetSuperAffordanceProviderMixin
...
from sims4.tuning.tunable import HasTunableReference, OptionalTunable, Tunable, TunableEnumEntry, TunableEnumFlags, TunableEnumWithFilter, TunableList, TunableMapping, TunableRange, TunableReference, TunableResourceKey, TunableSet, TunableTuple, TunableVariant
from sims4.tuning.tunable_base import ExportModes, GroupNames
from sims4.utils import classproperty, flexproperty
from singletons import DEFAULT
from situations.tunable import TunableSituationStart
from statistics.commodity import CommodityState, CommodityTimePassageFixupType, RuntimeCommodity
...
class Buff(HasTunableReference, RouteEventProviderMixin, SuperAffordanceProviderMixin, TargetSuperAffordanceProviderMixin, MixerActorMixin, MixerProviderMixin, metaclass = HashedTunedInstanceMetaclass, manager = services.get_instance_manager(sims4.resources.Types.BUFF)):
	INSTANCE_TUNABLES = {
		'buff_name': TunableLocalizedStringFactory(description = '\n        Name of buff.\n        ', allow_none = True, tuning_group = GroupNames.UI, export_modes = ExportModes.All),
		'buff_description': TunableLocalizedString(description = '\n        Tooltip description of the Buff Effect.\n        ', allow_none = True, tuning_group = GroupNames.UI, export_modes = ExportModes.All),
		'icon': TunableResourceKey(description = '\n        Icon to be displayed for buff\n        ', default = None, resource_types = sims4.resources.CompoundTypes.IMAGE, tuning_group = GroupNames.UI, export_modes = ExportModes.All),
		'ui_sort_order': Tunable(description = '\n        Order buff should be sorted in UI.\n        ', tunable_type = int, tuning_group = GroupNames.UI, default = 1, export_modes = ExportModes.All),
		...
		'portal_flags_override': TunableTuple(description = "\n            Override PortalFlags bits on a Sim's routing context,\n            which are used to allow sims to traverse portals that also have\n            these flags tuned on them, via the portal's Required Portal Flags\n            tuning.\n            ", set_flags = OptionalTunable(
			...
			TunableTuple(description = "\n                Tuple of allowance and discouragement flags to be removed \n                from the Sim's routing context for the duration of this buff.\n                ", portal_allowance_flags = TunableEnumFlags(description = '\n                    When this buff is present, bits tuned here will be unset on \n                    the Sim allowed portal flags.\n                    \n                    When the buff is removed, the tuned bits will be set.\n                    \n                    NOTE: This functionality does not care what the existing \n                    value of the bits are, and this will do exactly what it \n                    says, overwriting any previous value.\n                    ', enum_type = PortalFlags, allow_no_flags = True),
						 portal_discouragement_flags = TunableEnumFlags(description = '\n                    When this buff is present, bits tuned here will be unset on \n                    the Sim discouragement portal flags.\n                    \n                    When the buff is removed, the tuned bits will be set.\n                    \n                    NOTE: This functionality does not care what the existing \n                    value of the bits are, and this will do exactly what it \n                    says, overwriting any previous value.\n                    ', enum_type = PortalFlags, allow_no_flags = True))), tuning_group = GroupNames.SPECIAL_CASES),
		'teleport_cost_multiplier': OptionalTunable(description = '\n        If enabled, cost of the teleport will be affected by the multiplier.\n        ', tunable = Tunable(description = '\n            Float value to multiply the teleport cost whenever a teleport\n            action is triggered.\n            ', tunable_type = float, default = 1.0), tuning_group = GroupNames.SPECIAL_CASES),
		'vfx': TunableVariant(description = '\n        vfx to play on the sim when buff is added.\n        ', play_effect = PlayEffect.TunableFactory(), play_multiple_effects = PlayMultipleEffects.TunableFactory(), locked_args = {
			'no_effect': None
		}, default = 'no_effect', tuning_group = GroupNames.ANIMATION),
		'static_commodity_to_add': TunableSet(description = '\n        Static commodity that is added to the sim when buff is added to sim.\n        ', tunable = TunableReference(manager = services.static_commodity_manager(), class_restrictions = statistics.static_commodity.StaticCommodity, pack_safe = True)),
		...
		'timeout_string': TunableLocalizedStringFactory(
			...
			allow_none = True, tuning_group = GroupNames.UI, export_modes = (ExportModes.ClientBinary,)),
	...
	def on_add (self, from_load = False, apply_buff_loot = True):
		...
		if from_load or sim is not None:
			self._start_vfx()
	...
	def provide_route_events (self, route_event_context, sim, path, failed_types = None, **kwargs):
		...
		for route_event_cls in self.route_events.single_events:
			...
			if route_event_cls is not None and (route_event_context.route_event_already_scheduled(route_event_cls) or route_event_context.route_event_already_fully_considered(route_event_cls, self) or route_event_cls.test(resolver)):
				route_event_context.add_route_event(RouteEventType.BUFF_SINGLE, route_event_cls(provider = self, provider_required = True))
		for route_event_cls in self.route_events.repeating_events:
			...
			if route_event_cls is not None and (route_event_context.route_event_already_fully_considered(route_event_cls, self) or route_event_cls.test(resolver)):
				route_event_context.add_route_event(RouteEventType.BUFF_REPEAT, route_event_cls(provider = self, provider_required = True))
	...
	def _start_auto_situation (self):
		...
		resolver = SingleSimResolver(self._owner, additional_participants = {
			ParticipantType.PickedZoneId: frozenset()
		})
