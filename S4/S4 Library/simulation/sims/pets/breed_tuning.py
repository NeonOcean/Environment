...
from sims4.tuning.tunable import HasTunableReference, Tunable, TunableEnumEntry, TunableInterval, TunableList, TunableReference, TunableResourceKey, TunableSet, TunableTuple
...
class Breed(HasTunableReference, metaclass = HashedTunedInstanceMetaclass, manager = services.get_instance_manager(sims4.resources.Types.BREED)):
	INSTANCE_TUNABLES = {
		'breed_display_name': TunableLocalizedString(description = '\n            The breed display name.\n            ', export_modes = ExportModes.All),
		'breed_description': TunableLocalizedString(description = '\n            The breed description.\n            ', export_modes = ExportModes.All),
		'breed_species': TunableEnumEntry(description = '\n            This breed is restricted to this species.\n            ', tunable_type = SpeciesExtended, default = SpeciesExtended.HUMAN, invalid_enums = (SpeciesExtended.HUMAN,), binary_type = EnumBinaryExportType.EnumUint32, export_modes = ExportModes.All),
		'breed_tag': TunableTag(description = '\n            The tag associated with this breed.\n            ', filter_prefixes = BREED_TAG_FILTER_PREFIXES, pack_safe = False, export_modes = ExportModes.All),
		'breed_traits': TunableSet(description = '\n            Traits that are by default associated with this breed.\n            ', tunable = TunableReference(manager = services.get_instance_manager(Types.TRAIT)), export_modes = ExportModes.All),
		'breed_voices': TunableList(description = '\n            A list valid voice actors and pitch ranges that this breed can have\n            when randomly generated.\n            ',
									tunable = TunableTuple(breed_voice_actor_index = Tunable(description = "\n                    The breed's default voice actor is a combination of its species\n                    and index. In general, 0-3 is mapped to A-D, though not all\n                    species have all four.\n                    \n                    (The mapping of species+index to voice actor is maintained in\n                    CASSharedUtils.cpp.)\n        \n                    Dogs and small dogs share the same voice actors, and have four:\n                    Index 0 - DogA (Generic Dog)\n                    Index 1 - DogB (Small Yappy Dog)\n                    Index 2 - DogC (Tough Dog)\n                    Index 3 - DogD (Big Dumb Hound Dog)\n                    \n                    Cats have two:\n                    Index 0 - CatA (Generic Cat)\n                    Index 1 - CatB (Scratchy Alley Cat)\n                    ", tunable_type = int, default = 0),
														   breed_voice_pitch = TunableInterval(description = '\n                    Min/max voice pitch that the breed will have.\n                    ', tunable_type = float, default_lower = -1, default_upper = 1, minimum = -1, maximum = 1), export_class_name = 'BreedVoiceTuple'), export_modes = ExportModes.All),
