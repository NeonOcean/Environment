...
class OccultTracker:
    ...
    def _get_fallback_outfit_category(self, occult_type):
        if occult_type == OccultType.HUMAN:
            return OutfitCategory.EVERYDAY
        return self.OCCULT_DATA[occult_type].fallback_outfit_category
        ...
        if self._sim_info.has_outfit(current_outfit):
            ...
            if outfit_category != OutfitCategory.BATHING:
                if not self._sim_info.has_outfit((outfit_category, outfit_index)):
                    outfit_category = self._get_fallback_outfit_category(occult_type)
                    outfit_index = 0
    ...
    def _copy_shared_attributes(self, sim_info_a, sim_info_b, occult_type):
        ...
        fallback_outfit_category = self._get_fallback_outfit_category(occult_type)
    ...
    def apply_occult_genetics(self, parent_a, parent_b, seed, **kwargs):
        if self.sim_info.age < Age.TEEN and self.has_occult_type(OccultType.MERMAID):
            return
        r = random.Random()
        r.seed(seed)
        if r.random() < 0.5:
