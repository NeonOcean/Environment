    ...
    def __call__(self, test_targets=()):
        if self.match_type == MatchType.MATCH_ANY:
            for target in test_targets:
                ...
                if result:
                    return result
            return TestResult(False, 'No Sim matches the age criteria for ages {}', ', '.join(str(age) for age in self.ages), tooltip=self.tooltip)
        for target in test_targets:
            result = self._test_sim_info(target)
            if not result:
                return result
...
class TraitTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
    ...
    __slots__ = ('subject', 'whitelist_traits', 'blacklist_traits', 'num_whitelist_required', 'num_blacklist_allowed', 'apply_thresholds_on_individual_basis')
