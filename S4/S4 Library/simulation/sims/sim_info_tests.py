...
from interactions import ParticipantType, ParticipantTypeActorTargetSim, ParticipantTypeSim, ParticipantTypeSingle, ParticipantTypeSingleSim
...
from sims4.tuning.tunable import AutoFactoryInit, HasTunableSingletonFactory, OptionalTunable, Tunable, TunableEnumEntry, TunableEnumFlags, TunableEnumSet, TunableFactory, TunableList, TunablePackSafeReference, TunableRange, TunableReference, TunableSet, TunableSimMinute, TunableSkinTone, TunableThreshold, TunableVariant
...
class SimInfoTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
	...
	FACTORY_TUNABLES = {
		...
		'gender': OptionalTunable(tunable = TunableEnumEntry(description = '\n                The Sim must be of the specified gender.\n                ', tunable_type = Gender, default = None), enabled_name = 'specified', disabled_name = 'unspecified'),
		'ages': OptionalTunable(tunable = TunableEnumSet(description = '\n                The Sim must be one of the specified ages.\n                ', enum_type = Age, enum_default = Age.ADULT, default_enum_list = [Age.TEEN, Age.YOUNGADULT, Age.ADULT, Age.ELDER]), disabled_name = 'unspecified', enabled_name = 'specified'),
		'species': TunableVariant(specified = _SpeciesTestSpecies.TunableFactory(), specified_extended = _SpeciesTestExtendedSpecies.TunableFactory(), locked_args = {
			'unspecified': None
		}, default = 'unspecified'),
		'can_age_up': OptionalTunable(tunable = Tunable(description = '\n                Whether the Sim is eligible to advance to the next age.\n                ', tunable_type = bool, default = None), enabled_name = 'specified', disabled_name = 'unspecified'),
...
class BuffAddedTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
	...
	FACTORY_TUNABLES = {
		'acceptable_buffs': TunableSet(description = '\n            Buffs that will pass the test.\n            ', tunable = TunableReference(manager = services.get_instance_manager(sims4.resources.Types.BUFF), pack_safe = True)),
	...
	def get_expected_args (self):
		return {
			'buff': event_testing.test_constants.FROM_EVENT_DATA
		}
...
class GenealogyTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
	...
	def get_expected_args (self):
		return {
			'source_participants': self.subject,
...
class GenderPreferenceTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
	...
	def get_expected_args (self):
		return {
			'subject_participants': self.subject,
			'target_participants': self.target_sim
		}
...
class StoredObjectInfoTest(HasTunableSingletonFactory, AutoFactoryInit, BaseTest):
	...
	def get_expected_args (self):
		return {
			'subject_participant': self.subject,
