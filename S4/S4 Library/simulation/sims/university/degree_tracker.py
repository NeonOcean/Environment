...
class DegreeTracker(SimInfoTracker):
        ...
        if data.HasField('elective_timestamp'):
            ...
            for course_info in data.current_courses:
                ...
                if data.HasField('term_started_time'):
                    if self._sim_info.career_tracker is not None:
                        if course_info.course_slot is not None:
    ...
    def complete_term(self):
        ...
        term_gpa = round(term_grade_point / len(self._course_infos), 2)
    ...
    def _setup_start_of_term_alarm(self, time_till_start_term=None):
        ...
        if time_till_start_term is None:
            ...
            if not (time_till_start_term is None or alarm_data):
    ...
    def get_remaining_days_in_term(self):
        days = self._end_of_term_handle is not None and math.ceil(self._end_of_term_handle.get_remaining_time().in_days())
        if days == 1:
    ...
    def _get_courses_to_take(self, major, university, course_count, credit_count, courses=()):
        ...
        if self._current_university is not None:
            ...
            while core_class_count > 0:
                if start_index < major_course_count:
    ...
    def _find_replacement_course(self, courses_to_take):
        ...
        for course in core_courses:
            if course.guid64 not in self._previous_courses:
                if course not in courses_to_take:
                    if course.final_requirement_type == FinalCourseRequirement.EXAM:
                        return course
        ...
        while elective_list:
            ...
            if course_data.final_requirement_type == FinalCourseRequirement.EXAM:
                if course_id not in self._previous_courses:
                    return course_data
