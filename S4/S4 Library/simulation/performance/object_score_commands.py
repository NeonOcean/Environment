...
import services
import sims4
POINTS_PER_INTERACTION = -0.000148017
POINTS_PER_AUTONOMOUS_INTERACTION = 7.7106e-05
POINTS_PER_PROVIDED_POSTURE_INTERACTION = -0.000337676
POINTS_PER_CLIENT_STATE_TUNING = 0.002176947
POINTS_PER_CLIENT_STATE_CHANGE_TUNING = 6.31107e-10
POINTS_PER_PORTAL_COMPONENT = 7.21342e-06
POINTS_PER_STATISTIC = -0.003321775
POINTS_PER_COMMODITY = 0.000956718
POINTS_PER_INVENTORY_COMPONENT = 0.001692892
POINTS_PER_OBJECT_PART = 0
POINTS_PER_INVENTORY_ITEM_COMPONENT = 0
...
def dump_object_scores(_connection=None):
    ...
    def _score_objects_callback(file):
        file.write('Object,total,On Lot,Off Lot,Interaction,Autonomous,provided posture,state component,client change tuning,parts,stats,commodities,portal,inventory item, inventory\n')
...
def show_lot_score(show_load_time:bool=True, _connection=None):
    ...
    if show_load_time and not indexed_manager.capture_load_times:
        sims4.commands.cheat_output('To see lot load time please use the |performance.toggler_object_load_capture and reload the lot.', _connection)
    elif show_load_time:
...
def _score_all_objects(object_score_counter):
    ...
    for obj in all_objects:
        ...
        if obj.has_component(PORTAL_COMPONENT):
            ...
            object_score_counter[obj_type]['portal_component'] += len(portal_instances)*POINTS_PER_PORTAL_COMPONENT
        if obj.has_component(INVENTORY_ITEM_COMPONENT):
            object_score_counter[obj_type]['inventory_item_component'] += 1*POINTS_PER_INVENTORY_ITEM_COMPONENT
        if obj.has_component(INVENTORY_COMPONENT):
            object_score_counter[obj_type]['inventory_component'] += 1*POINTS_PER_INVENTORY_COMPONENT
