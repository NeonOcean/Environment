...
def show_lot_score(show_load_time:bool=True, _connection=None):
    object_scores = defaultdict(Counter)
    (on_lot_objects, off_lot_objects) = _score_all_objects(object_scores)
    ...
    for obj in all_objects:
        ...
        for super_affordance in obj.super_affordances():
            if not super_affordance.allow_autonomous:
                object_score_counter[obj_type]['interaction'] += _convert_score_to_percentage_of_max(POINTS_PER_INTERACTION)
            else:
                object_score_counter[obj_type]['autonomous'] += _convert_score_to_percentage_of_max(POINTS_PER_AUTONOMOUS_INTERACTION)
            if super_affordance.provided_posture_type is not None:
                object_score_counter[obj_type]['provided_posture'] += _convert_score_to_percentage_of_max(POINTS_PER_PROVIDED_POSTURE_INTERACTION)
        if obj.has_component(STATE_COMPONENT):
            for (_, client_state_values) in obj.get_component(STATE_COMPONENT)._client_states.items():
                ...
                object_score_counter[obj_type]['state_component'] += 1*_convert_score_to_percentage_of_max(POINTS_PER_CLIENT_STATE_TUNING)
                if client_change_op_count > 0:
                    object_score_counter[obj_type]['client_change_tuning'] += client_change_op_count*_convert_score_to_percentage_of_max(POINTS_PER_CLIENT_STATE_CHANGE_TUNING)
        if obj.parts:
            object_score_counter[obj_type]['parts'] += len(obj.parts)*_convert_score_to_percentage_of_max(POINTS_PER_OBJECT_PART)
        if obj.statistic_tracker is not None:
            object_score_counter[obj_type]['statistics'] += len(obj.statistic_tracker)*_convert_score_to_percentage_of_max(POINTS_PER_STATISTIC)
        if obj.commodity_tracker is not None:
            object_score_counter[obj_type]['commodities'] += len(obj.commodity_tracker)*_convert_score_to_percentage_of_max(POINTS_PER_COMMODITY)
        if obj.has_component(PORTAL_COMPONENT):
            ...
            object_score_counter[obj_type]['portal_component'] += len(portal_instances)*_convert_score_to_percentage_of_max(POINTS_PER_PORTAL_COMPONENT)
        if obj.has_component(INVENTORY_ITEM_COMPONENT):
            object_score_counter[obj_type]['inventory_item_component'] += 1*_convert_score_to_percentage_of_max(POINTS_PER_INVENTORY_ITEM_COMPONENT)
        if obj.has_component(INVENTORY_COMPONENT):
            object_score_counter[obj_type]['inventory_component'] += 1*_convert_score_to_percentage_of_max(POINTS_PER_INVENTORY_COMPONENT)
...
def _dump_object_to_file(object_type, object_scores, on_lot_objects, off_lot_objects, file, verbose=True):
    object_counter = object_scores[object_type]
    file.write('{},{:.02}%,{},{}'.format(object_type, sum(object_counter.values()), on_lot_objects[object_type], off_lot_objects[object_type]))
    if not verbose:
        file.write('\n')
        return
...
def _dump_total_score(object_scores, on_lot_objects, off_lot_objects, file):
    total_score = 0
