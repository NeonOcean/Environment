...
from interactions.payment.payment_dest import PaymentDestActiveHousehold, PaymentDestBusiness, PaymentDestNone, PaymentDestParticipantHousehold, PaymentDestStatistic, PaymentDestTuningFlags
from interactions.payment.payment_info import BusinessPaymentInfo, PaymentBusinessRevenueType, PaymentInfo
...
class _Payment(HasTunableSingletonFactory, AutoFactoryInit):
	...
	def payment_destination_option (available_dest_flags = PaymentDestTuningFlags.ALL):
		dest_kwargs = {
			'no_dest': PaymentDestNone.TunableFactory()
		}
		default = 'no_dest'
...
class _PaymentWrapper(_Payment):
	FACTORY_TUNABLES = {
		'wrapped_cost': TunableVariant(description = '\n            The amount to pay, affected by wrapped payment type. If this is 0,\n            then this operation costs nothing.\n            ', amount = PaymentAmount.TunableFactory(), amount_up_to = PaymentAmountUpTo.TunableFactory(), catalog_value = PaymentCatalogValue.TunableFactory(), current_value = PaymentCurrentValue.TunableFactory(), base_retail_value = PaymentBaseRetailValue.TunableFactory(), default = 'amount'),
		'locked_args': {
			'payment_destinations': ()
		}
	}
...
class PaymentDialog(_Payment):
	FACTORY_TUNABLES = {
		'input_dialog': UiDialogTextInputOkCancel.TunableFactory(description = '\n            The dialog that is displayed. The amount the user enters into the\n            input is used as the payment amount.\n            ', text_inputs = (TEXT_INPUT_PAYMENT_VALUE,)),
