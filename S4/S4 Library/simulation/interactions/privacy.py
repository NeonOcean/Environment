...
from sims4.tuning.tunable import TunableFactory, Tunable, TunableReference, OptionalTunable, TunableTuple
    ...
    def check_for_late_violators(self, sim):
        for privacy in self.privacy_instances:
            if not sim in privacy.violators:
                ...
                elif sim not in privacy.find_violating_sims():
                    pass
                else:
...
class Privacy(LineOfSight):
    ...
    def __init__(self, interaction, tests, shoo_exempt_tests, max_line_of_sight_radius, map_divisions, simplification_ratio, boundary_epsilon, facing_offset, routing_surface_only, shoo_constraint_radius, unavailable_tooltip, embarrassed_affordance, reserved_surface_space, vehicle_tests):
    ...
    def _is_sim_allowed(self, sim):
        if self._tests:
            ...
            if self._tests and self._tests.run_tests(resolver):
                return True
            elif self._interaction.can_sim_violate_privacy(sim):
                return True
        elif self._interaction.can_sim_violate_privacy(sim):
            return True
        return False
    ...
    def build_privacy(self, target=None):
        ...
        for poly in self.constraint.geometry.polygon:
            self._privacy_constraints.append(PolygonFootprint(poly, routing_surface=routing_surface, cost=self._PRIVACY_DISCOURAGEMENT_COST, footprint_type=self._PRIVACY_FOOTPRINT_TYPE, enabled=True))
        if self._reserved_surface_space is not None and target is not None:
            ...
            footprint_cost = self._PRIVACY_DISCOURAGEMENT_COST if self._reserved_surface_space.allow_routing else self._PRIVACY_SURFACE_BLOCKING_FOOTPRINT_COST
