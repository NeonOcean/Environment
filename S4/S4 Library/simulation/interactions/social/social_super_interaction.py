...
class SocialSuperInteraction(SocialInteractionMixin, SocialCompatibilityMixin, SuperInteraction):
    ...
    def __init__(self, *args, initiated=True, social_group=None, source_social_si=None, set_work_timestamp=False, **kwargs):
        SuperInteraction.__init__(self, *args, initiated=initiated, set_work_timestamp=set_work_timestamp, **kwargs)
        SocialInteractionMixin.__init__(self, *args, initiated=initiated, **kwargs)
    ...
    def _get_close_to_target_and_greet(self, force=False):
        ...
        try:
            ...
            if sim_posture.multi_sim and sim_posture.linked_sim is target_sim:
                return False
            constraint_cone = greetings.GreetingsSatisfyContraintTuning.CONE_CONSTRAINT.create_constraint(self.sim, target_sim, target_position=target_sim_position, target_forward=target_sim.intended_forward, target_routing_surface=target_sim_routing_surface)
    ...
    def _constraint_gen(cls, inst, sim, target, participant_type=ParticipantType.Actor):
        ...
        if inst is None or inst.social_group is None or not inst.social_group.constraint_initialized:
            ...
            if initiator.si_state.is_compatible_constraint(fallback_constraint, priority=priority, to_exclude=inst) and recipient.si_state.is_compatible_constraint(fallback_constraint, priority=priority, to_exclude=inst):
    ...
    def _choose_social_group(self):
        ...
        if social_group is None:
            ...
            self._social_group = social_group
            if self.target.is_sim and self._update_social_geometry_if_target_moves:
