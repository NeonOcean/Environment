...
from interactions import ParticipantType, ParticipantTypeSim, ParticipantTypeSingleSim, PipelineProgress, TargetType
from interactions.aop import AffordanceObjectPair
from interactions.base.basic import TunableBasicContentSet
from interactions.base.interaction import CancelGroupInteractionType, Interaction
...
from interactions.constraints import Anywhere, PostureConstraintFactory, create_constraint_set
...
from interactions.utils.interaction_liabilities import CANCEL_AOP_LIABILITY, CANCEL_INTERACTION_ON_EXIT_LIABILITY, CancelAOPLiability, CancelInteractionsOnExitLiability, OWNS_POSTURE_LIABILITY, OwnsPostureLiability
...
from postures import DerailReason, PostureTrack, PostureTransitionTargetPreferenceTag, posture_graph
...
from sims.outfits.outfit_change import InteractionOnRouteOutfitChange, TunableOutfitChange
from sims.outfits.outfit_enums import DefaultOutfitPriority, OutfitChangeReason
...
from sims4.tuning.geometric import TunablePolygon, TunableVector2, TunableVector3, TunableWeightedUtilityCurveAndWeight
from sims4.tuning.tunable import AutoFactoryInit, HasTunableSingletonFactory, OptionalTunable, Tunable, TunableAngle, TunableEnumEntry, TunableEnumSet, TunableList, TunableMapping, TunableReference, TunableSet, TunableSimMinute, TunableTuple, TunableVariant
from sims4.tuning.tunable_base import GroupNames
from sims4.utils import classproperty, flexmethod, flexproperty
from singletons import DEFAULT
from snippets import ANIMATION_ACTOR_MAP, TunableAffordanceFilterSnippet, define_snippet
...
class SuperInteraction(RouteEventProviderMixin, Interaction, StageControllerElement):
	...
	DEFAULT_POSTURE_TARGET_PREFERENCES = TunableMapping(
	...
	INSTANCE_TUNABLES = {
													   ...
													   description = '\n                            If enabled, specifies a posture constraint that the standard\n                            constraint must be compatible with for these constraints\n                            to be applied.  i.e. if you only want the constraints\n                            to be applied to elements of the normal constraint set\n                            that are sitting, you would specify a sitting constraint here.\n                            ', tunable = PostureConstraintFactory.TunableFactory()), constraint_infos = TunableList(description = '\n                            Specifies the constraints and target of the constraint to apply.\n                            ',
																																																																																																																																																											   tunable = TunableTuple(constraints = TunableList(tunable = TunableGeometricConstraintVariant(description = '\n                                        A constraint that must be fulfilled in order to interact\n                                        with this object.\n                                        '), minlength = 1),
																																																																																																																																																																	  constraint_target = TunableVariant(description = '\n                                    The object used to generate constraints relative to.\n                                    ', participant_based = TunableTuple(participant = TunableEnumEntry(
																																																																																																																																																																		  ...
																																																																																																																																																																		  tunable_type = ParticipantType, default = ParticipantType.Object), locked_args = {
																																																																																																																																																																		  'is_participant': True
																																																																																																																																																																	  }), tag_based = TunableTuple(
																																																																																																																																																																		  tags = TunableSet(description = '\n                                            The tag for objects used to generate _constraints relative to.\n                                            ', tunable = TunableEnumEntry(tunable_type = Tag, default = Tag.INVALID, invalid_enums = (Tag.INVALID,), pack_safe = True), minlength = 1), locked_args = {
																																																																																																																																																																			  'is_participant': False
																																																																																																																																																																		  }), default = 'participant_based')))))), tuning_group = GroupNames.CORE),
		...
		'posture_target_preference': OptionalTunable(TunableMapping(
			...
			key_type = TunableEnumEntry(PostureTransitionTargetPreferenceTag, PostureTransitionTargetPreferenceTag.INVALID), value_type = Tunable(float, 0)), tuning_group = GroupNames.POSTURE),
		...
		'scoring_priority': TunableEnumEntry(
			...
			tunable_type = autonomy.autonomy_interaction_priority.AutonomyInteractionPriority, default = autonomy.autonomy_interaction_priority.AutonomyInteractionPriority.INVALID, needs_tuning = True, tuning_group = GroupNames.AUTONOMY),
		'basic_content': TunableBasicContentSet(description = '\n                The main animation and periodic stat changes for the\n                interaction.\n                ', one_shot = True, flexible_length = True, default = 'flexible_length', tuning_group = GroupNames.CORE),
								 ...
								 tunable = TunableTuple(tag = TunableEnumEntry(description = '\n                        An identifying tag that determines how consecutive\n                        rallyable interactions are grouped and handled.\n                        ', tunable_type = RallyableTag, default = None), sources = TunableEnumSet(description = '\n                        A list of different rally sources that we want to offer\n                        this rally from.\n                        ', enum_type = RallySource, enum_default = RallySource.ENSEMBLE, default_enum_list = frozenset((RallySource.ENSEMBLE,))), pie_menu_icon = OptionalTunable(description = '\n                        If enabled then we will use this pie menu icon on the\n                        interaction.\n                        ', tunable = TunableIconVariant(description = '\n                            The icon to display in the pie menu.\n                            ', icon_pack_safe = True)),
														skip_interaction_test = Tunable(description = '\n                        If checked then we will skip testing the Sims we will\n                        rally, with the tests of this interaction.\n                        ', tunable_type = bool, default = False), behavior = TunableVariant(description = "\n                        Select the behavior this interaction will have with\n                        respect to members of the Sim's Party.\n                        ", push_affordance = TunableTuple(description = '\n                            Bring the Party along and push the specified \n                            interaction on all members.\n                            ', loc_display_name = TunableLocalizedStringFactory(default = 3390898100),
																																																																																																																																												affordance_target = OptionalTunable(description = '\n                                The target of the pushed affordance relative to\n                                original interaction. so actor would be sim\n                                that triggered the rally. Use None for\n                                affordances like sit-smart.\n                                ',
																																																																																																																																																					tunable = TunableEnumEntry(ParticipantType, ParticipantType.Object), enabled_by_default = True, disabled_name = 'none', enabled_name = 'participant_type'),
		...
		'acquire_targets_as_resource': Tunable(
			...
			default = True, tuning_group = GroupNames.SPECIAL_CASES),
	...
	def _tuning_loaded_callback (cls):
		...
		if cls._provided_posture_type is not None:
			...
			cls._provided_posture_type_disallowed_ages = frozendict({
																		cls._provided_posture_type_species: disallowed_ages
																	})
	...
	def _verify_tuning_callback (cls):
		...
		for affordance in cls.all_affordances_gen():
			...
			if not affordance.optional:
				if not has_valid_intersection:
					for constraint_error in constraint_errors:
						logger.error(constraint_error)
		if cls.score_additional_sims_for_in_use is not None and cls.basic_reserve_object is None:
	...
	def get_provided_posture_change (cls, aop):
		if cls._provided_posture_type is not None:
			return PostureOperation.BodyTransition(cls._provided_posture_type, enumdict(Species, {
				cls._provided_posture_type_species: aop
			}), target = aop.target, disallowed_ages = cls._provided_posture_type_disallowed_ages)
			...
			if not (test_self and si_affordance.get_interaction_type() is cls_interaction_type):
				if si_affordance in cls.test_disallow_while_running.affordances:
					...
					if si.target is not None:
						if si.target.is_part:
							if si.target.part_owner is target:
								if not is_starting:
									return TestResult(False, 'Currently running interaction')
			if 'interaction_starting' in interaction_parameters:
				is_starting = interaction_parameters['interaction_starting']
	...
	def _constraint_gen (cls, inst, sim, target, participant_type = ParticipantType.Actor, **kwargs):
		...
		for constraint in super(SuperInteraction, inst_or_cls)._constraint_gen(sim, inst_or_cls.get_constraint_target(target), participant_type = participant_type, **kwargs):
			...
			yield constraint
		if inst is not None and (inst.sim.posture.source_interaction is inst and not inst.sim.posture.mobile) and not inst.sim.posture.is_universal:
		...
		for tuned_additional_constraint in target.additional_interaction_constraints:
			...
			found_constraint = True
			if not (affordance_links is None or not affordance_links(cls)) and constraint is not None:
		...
		if carry_participant is not None and carry_participant != ParticipantType.CreatedObject:
			...
			if carry_target is not None:
				if self.context.carry_target is None:
					...
					if carry_target != self.context.carry_target:
	...
	def get_combinable_interactions_with_safe_carryables (self):
		...
		for combinable in self.combinable_interactions:
			...
			if combinable_carry is not None and combinable_carry not in combined_carry_targets:
				if not isinstance(combinable_carry, Definition) and combinable_carry.is_sim:
					continue
	...
	def queued_sub_interactions_gen (self):
		for interaction in self.sim.queue:
			if not interaction.is_super:
				if interaction.super_interaction is self:
					yield interaction
		...
		for route_event_tuple in self.route_events:
			...
			for participant in participants:
				if not failed_types is None:
					pass
				if participant is sim and (not route_event_context.route_event_already_scheduled(route_event_tuple.route_event) and route_event_tuple.route_event is not None) and route_event_tuple.route_event.test(resolver):
	...
	def prepare_gen (self, timeline, cancel_incompatible_carry_interactions = False):
		...
		if self.waiting_line is not None and not self._ignore_waiting_line_tuning and (self.target is not current_posture_target or self.waiting_line.allow_line_on_same_target):
	...
	def _maybe_acquire_posture_ownership (self):
		...
		if self.is_finishing:
			return
		if not self.disable_transitions and (not self.is_compatible_with_stand_at_none and self.target is not None) and self.carry_target is None:
	...
	def _setup_gen (self, timeline):
		...
		if not self.disable_transitions and self not in self.si_state:
			...
			if new_carry_owner is not None and posture.target is carry_target:
				if head_interaction not in posture.owning_interactions:
					...
					if proxy_posture_owner is None:
						if cancel_affordance is not None:
						...
						elif posture.source_interaction is self:
							if cancel_affordance is not None:
								cancel_replacement_aop = AffordanceObjectPair(cancel_affordance, target, cancel_affordance, None)
								replacement_aop = cancel_replacement_aop
							elif posture.track == PostureTrack.BODY:
								replacement_aop = posture_graph.PostureGraphService.get_default_aop(self.sim.species)
							elif posture.target is not None:
								if posture.target.valid_for_distribution:
									if posture.target.reset_reason != ResetReason.BEING_DESTROYED:
							...
							context = self._get_cancel_replacement_context_for_posture(posture, replacement_aop.affordance, always_run)
							cancel_aops_contexts_postures.append((replacement_aop, context, posture))
					if not posture.last_owning_interaction(self) or posture.source_interaction is self:
						if cancel_affordance is not None:
							cancel_replacement_aop = AffordanceObjectPair(cancel_affordance, target, cancel_affordance, None)
							replacement_aop = cancel_replacement_aop
						elif posture.track == PostureTrack.BODY:
	...
	def _try_exit_via_cancel_aop (self, carry_cancel_override = None):
		...
		for (on_cancel_aop, context, posture) in on_cancel_aops_contexts_postures:
			if on_cancel_aop.affordance is None:
				continue
			if context.source == InteractionSource.BODY_CANCEL_AOP and sim.parent is not None and sim.parent.is_sim:
				continue
	...
	def _cancel_eventually (self, *args, immediate = False, **kwargs):
		if not self.is_finishing and not not (self._cancel_deferred is not None and immediate):
	...
	def set_stat_asm_parameter (self, asm, actor_name, target_name, sim, target):
		...
		def set_asm_param (skill_stat, skill_sim, skill_actor_name):
			if skill_sim is None or not skill_sim.is_sim:
				return
			if skill_stat is None or skill_actor_name is None:
				return
