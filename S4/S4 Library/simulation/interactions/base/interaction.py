...
from interactions.liability import Liability, ReplaceableLiability, SharedLiability
...
from sims4.tuning.tunable import HasTunableReference, OptionalTunable, Tunable, TunableEnumEntry, TunableList, TunableMapping, TunableRange, TunableReference, TunableSet, TunableSimMinute, TunableThreshold, TunableTuple, TunableVariant
from sims4.tuning.tunable_base import GroupNames
from sims4.utils import classproperty, constproperty, flexmethod, flexproperty, setdefault_callable
...
class OwnsPostureLiability(ReplaceableLiability):
	...
	def should_transfer (self, continuation):
		if not continuation.involves_carry and self._posture_ref is not None and isinstance(self._posture_ref(), CarryPosture):
			return False
		return True
...
class Interaction(HasTunableReference, ContentScoreMixin, metaclass = HashedTunedInstanceMetaclass, manager = services.get_instance_manager(sims4.resources.Types.INTERACTION)):
	...
	INSTANCE_TUNABLES = {
		...
		'item_cost': ItemCost.TunableFactory(tuning_group = GroupNames.SPECIAL_CASES),
		...
		try:
			...
			if result:
				...
				if household is not None:
					result &= services.utilities_manager(household.id).test_utility_info(cls.utility_info)
			if result:
	...
	def set_target (self, target):
		...
		if self.target_type & TargetType.ACTOR or self.target_type & TargetType.FILTERED_TARGET:
			target = None
		elif target is self.sim and not self.immediate:
	...
	def build_basic_content (self, sequence = (), **kwargs):
		if self.basic_content is not None:
			...
			sequence = (sequence, self._set_last_animation_factory)
		if self.target is not None:
			target_basic_content = self.target.get_affordance_basic_content(self)
			if target_basic_content is not None:
			...
			for (_, var_map, _) in posture_specs:
				...
				if hand is not None:
					...
					if temp_carry_posture.is_two_handed_carry:
						...
						break
			forced_tentative_interaction_constraint = None if not carry_target is not None or not carry_target.is_sim or posture_state is not None else current_interaction_constraint
			for (carry_hand, carry_posture, carry_object) in get_carried_objects_gen(sim):
				if carry_object is not carry_target and carry_object is not interaction_target:
