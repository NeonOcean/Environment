...
from interactions import ParticipantType, ParticipantTypeSavedActor, ParticipantTypeSingleSim, ParticipantTypeSituationSims, PipelineProgress, TargetType
...
from sims4.tuning.tunable import HasTunableReference, OptionalTunable, Tunable, TunableEnumEntry, TunableList, TunableMapping, TunablePackSafeReference, TunableRange, TunableReference, TunableSet, TunableSimMinute, TunableThreshold, TunableTuple, TunableVariant
...
class Interaction(HasTunableReference, ContentScoreMixin, metaclass = HashedTunedInstanceMetaclass, manager = services.get_instance_manager(sims4.resources.Types.INTERACTION)):
	...
	INSTANCE_TUNABLES = {
		...
		'utility_info': OptionalTunable(TunableList(description = '\n                Tuning that specifies which utilities this interaction requires\n                to be run.\n                ', tunable = TunableEnumEntry(Utilities, None)), tuning_group = GroupNames.AVAILABILITY),
	...
	def test (cls, inst, target = DEFAULT, context = DEFAULT, super_interaction = None, skip_safe_tests = False, **interaction_parameters):
		...
		try:
			...
			if result:
				if not target or target.is_on_active_lot():
	...
	def get_participants (cls, inst, participant_type: ParticipantType, sim = DEFAULT, target = DEFAULT, carry_target = DEFAULT, listener_filtering_enabled = False, target_type = None, **interaction_parameters) -> set:
		...
		if not sim is not None or participant_type == ParticipantType.TargetSimPostureTarget:
			if target is not None and target.is_sim:
	...
	def _update_autonomy_timer (self, force_user_directed = False):
		...
		if force_user_directed or self.is_user_directed:
			...
			if not self.is_social:
				return
			target = self.target
			if target is not None and target.is_sim and not target.is_being_destroyed:
