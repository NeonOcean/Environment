...
from animation.animation_utils import flush_all_animations, with_event_handlers
from animation.arb_element import distribute_arb_element
from animation.posture_manifest import AnimationParticipant, MATCH_ANY, MATCH_NONE, PostureManifest, PostureManifestEntry
...
from carry.carry_utils import create_two_handed_carry_constraint, get_carried_objects_gen, hand_to_track, holster_carried_object, interact_with_carried_object, set_carry_track_param_if_needed
...
from interactions import ParticipantType, ParticipantTypeObject, ParticipantTypeSavedActor, ParticipantTypeSingleSim, ParticipantTypeSituationSims, PipelineProgress, TargetType
from interactions.base.basic import AFFORDANCE_LOADED_CALLBACK_STR, TunableBasicContentSet, TunableBasicExtras
...
from interactions.utils.interaction_liabilities import ANIMATION_CONTEXT_LIABILITY, AUTONOMY_MODIFIER_LIABILITY, AnimationContextLiability, AutonomyModifierLiability, RESERVATION_LIABILITY, STAND_SLOT_LIABILITY, UNCANCELABLE_LIABILITY
...
from interactions.utils.sim_focus import SimFocus, TunableFocusElement, with_sim_focus
from interactions.utils.statistic_element import ConditionalInteractionAction, ExitCondition
...
from interactions.utils.tunable import CriticalPriorityLiability, GameSpeedLiability, PushAffordanceOnRouteFailLiability, SaveLockLiability, TimeoutLiability, TunableContinuation, TunableStatisticAdvertisements
...
from sims.funds import FundsSource, FundsTuning, get_funds_for_source
...
from sims4.tuning.tunable import HasTunableReference, OptionalTunable, Tunable, TunableEnumEntry, TunableEnumSet, TunableList, TunableMapping, TunablePackSafeReference, TunableRange, TunableReference, TunableSet, TunableSimMinute, TunableThreshold, TunableTuple, TunableVariant
from sims4.tuning.tunable_base import GroupNames
from sims4.utils import classproperty, constproperty, flexmethod, flexproperty
...
class Interaction(HasTunableReference, ContentScoreMixin, metaclass = HashedTunedInstanceMetaclass, manager = services.get_instance_manager(sims4.resources.Types.INTERACTION)):
	...
	def register_tuned_animation (cls, interaction_asm_type, asm_key, actor_name, target_name, carry_target_name, create_target_name, overrides, participant_type_actor, participant_type_target):
		...
		if participant_type_target is not None:
	...
	def _statistic_operations_gen (cls):
		...
		for operation_list in cls.basic_content.periodic_stat_change.operation_actions.actions:
			for (op, _) in operation_list.get_loot_ops_gen():
				yield op
		if not cls.basic_content.periodic_stat_change is not None or cls.basic_content.progressive_stat_change is not None:
	...
	def _tuning_loaded_callback (cls):
		for op in cls._statistic_operations_gen():
			if op.advertise:
				if op.subject == ParticipantType.Actor:
	...
	def test (cls, inst, target = DEFAULT, context = DEFAULT, super_interaction = None, skip_safe_tests = False, **interaction_parameters):
		...
		try:
			...
			if target is not None:
				...
				if not cls.simless and not (target.parent is not None and (target.parent.is_sim and (target.parent is not context.sim and (target is not context.sim and not target.is_set_as_head))) and not target.is_sim):
			...
			if (not inst_or_cls is not None or (not inst_or_cls.test_incest or not target is not None) or inst_or_cls is not None) and not inst_or_cls.debug:
			...
			if not instance_result and not instance_result.tooltip:
			...
			if inst_or_cls._tunable_tests_enabled():
				...
				if not context.shift_held:
					if inst_or_cls.is_super:
						if not context.is_cancel_aop:
				...
				if global_result or search_for_tooltip:
					if global_result.tooltip is not None:
						...
						if local_result or search_for_tooltip:
							if local_result.tooltip is not None:
			...
			if result:
				if cls.utility_info is not None:
					if not target_of_interest or target_of_interest.is_on_active_lot():
						if resolver is None:
			...
			if result:
				if not (context.sim is None or not context.sim.is_npc):
	...
	def get_participants (cls, inst, participant_type: ParticipantType, sim = DEFAULT, target = DEFAULT, carry_target = DEFAULT, listener_filtering_enabled = False, target_type = None, **interaction_parameters) -> set:
						...
						for other_sim in social_group:
							...
							for si in social_group.get_sis_registered_for_sim(other_sim):
								if si.pipeline_progress >= PipelineProgress.RUNNING and not si.is_finishing:
									break
							if not inst.acquire_listeners_as_resource and listener_filtering_enabled and other_sim.ignore_group_socials(excluded_group = social_group):
								pass
		...
		if participant_type & ParticipantType.ObjectInventoryOwner and target is not None and target.is_in_inventory():
			...
			if owner is not None:
				result.add(owner)
		if participant_type & ParticipantType.ObjectIngredients and (target is not None and not target.is_sim) and target.crafting_component:
		...
		if (not sim is not None or not hasattr(sim, 'posture_state') or target is not None) and target.is_sim and hasattr(target, 'posture_state'):
		...
		if participant_type & ParticipantType.CreatedObject or participant_type & ParticipantType.CreatedObjectIngredients:
			if inst is not None and inst.created_target is not None:
				...
				if participant_type & ParticipantType.CreatedObjectIngredients:
				...
				if inst.interaction_parameters:
		...
		if participant_type & ParticipantType.TargetFeudTarget and target is not None and target.is_sim:
			...
			if feud_target is not None:
				result.add(feud_target)
		if participant_type & ParticipantType.ActorSquadMembers:
			if sim is not None:
		...
		if participant_type & ParticipantType.TargetSquadMembers:
			if target is not None:
				if target.is_sim:
					sim_info_manager = services.sim_info_manager()
					for squad_member_id in target.squad_members:
						...
						if squad_sim_info is not None:
							result.add(squad_sim_info)
		if participant_type & ParticipantType.StoredObjectsOnActor and sim is not None:
			actor = sim.sim_info
	...
	def get_participant_type (cls, inst, participant, restrict_to_participant_types = None, exclude_participant_types = (), **kwargs) -> ParticipantType:
		...
		for participant_type in priority_participant_types:
			...
			if participant_type in exclude_participant_types:
				continue
			if participant in inst_or_cls.get_participants(participant_type, **kwargs):
	...
	def get_simoleon_cost_for_source (cls, inst, funds_source, target = DEFAULT, context = DEFAULT, **interaction_parameters):
		inst_or_cls = inst if inst is not None else cls
		return -sum(amount for (amount, _funds_source) in inst_or_cls.get_simoleon_deltas_gen(target, context, **interaction_parameters) if amount < 0 if _funds_source == funds_source)
	...
	def get_name (cls, inst, target = DEFAULT, context = DEFAULT, apply_name_modifiers = True, **interaction_parameters):
		...
		if apply_name_modifiers:
			...
			if wrapper_item is not None:
				display_name = wrapper_item.wrapper(display_name)
			if inst_or_cls.debug:
	...
	def autonomy_ads_gen (cls, target = None, include_hidden_false_ads = False):
		...
		for ad in cls._hidden_false_advertisements:
			cls._add_autonomy_ad(ad, overwrite = False)
		if not (not target is not None or include_hidden_false_ads) or cls._autonomy_ads:
	...
	def constraint_intersection (cls, inst, sim = DEFAULT, target = DEFAULT, participant_type = DEFAULT, *, posture_state = DEFAULT, force_concrete = True, allow_holster = DEFAULT, invalid_expected = False):
		...
		if intersection is None:
			...
			for constraint in constraints:
				...
				if not intersection.valid:
					break
			if not inst_or_cls.is_super or inst is not None:
		...
		for (key, liability) in list(self._liabilities.items()):
			if liabilities_to_release:
				if key in liabilities_to_release:
					if continuation is not None:
						if liability.should_transfer(continuation):
							if not (key != ANIMATION_CONTEXT_LIABILITY or not continuation.ignore_animation_context_liability):
		...
		def resolver (constraint_participant, default = None):
			...
			if constraint_participant == AnimationParticipant.ACTOR:
				...
				if posture_state is not None:
					...
					if result is None:
						result = MATCH_NONE
				elif constraint_participant == AnimationParticipant.SURFACE:
					if default == AnimationParticipant.SURFACE:
	...
	def perform_gen (self, timeline):
		...
		if not self.disable_transitions:
			...
			assert not constraint_interaction.tentative
			if not self.is_super and not not (self.super_interaction is not None and constraint_interaction.valid):
	...
	def _pre_perform (self):
		...
		curfew_service = services.get_curfew_service()
		if curfew_service.sim_breaking_curfew(self.sim, self.target, self):
	...
	def build_basic_elements (self, sequence = (), **kwargs):
		...
		if self.basic_focus is not None:
			if self.provided_posture_type is None:
		...
		template_affordance_tracker = self.sim.sim_info.template_affordance_tracker if self.sim is not None else None
			...
			for item in actor.inventory_component:
				if check_type == TunableContinuation.ITEM_DEFINITION:
					...
					if item.definition.tuning_file_id == inventory_carry_target.definition.tuning_file_id:
						context.carry_target = item
						break
				elif check_type == TunableContinuation.TAGGED_ITEM:
					if inventory_carry_target(item, None):
						context.carry_target = item
						break
	...
	def cancel (self, finishing_type, cancel_reason_msg, ignore_must_run = False, **kwargs):
		...
		if self.is_finishing or not not (self.must_run and ignore_must_run):
			return False
	...
	def cancel_user (self, cancel_reason_msg, on_response = None):
		...
		if self.get_liability(UNCANCELABLE_LIABILITY) is not None:
			return False
		if self._cancelable_by_user == CancelablePhase.NEVER or not not (self._cancelable_by_user == CancelablePhase.RUNNING and self.running):
			_send_response(False)
			return False
		if self._cancelable_by_user == CancelablePhase.ALWAYS or not (self._cancelable_by_user == CancelablePhase.RUNNING and self.running or not self.prepared):
	...
	def allow_outcomes (self):
		...
		if self.is_basic_content_one_shot:
			return True
		if not self.is_finishing and self._finisher.has_pending_natural_finisher:
			return True
	...
	def cancel_incompatible_carry_interactions (self, can_defer_putdown = True, derail_actors = False):
		...
		for (owning_interaction, carry_posture) in self.get_uncarriable_objects_gen(posture_state = None):
			if can_defer_putdown and carry_posture.target.carryable_component.defer_putdown:
				continue
			if not owning_interaction.is_cancel_aop:
				if derail_actors or not self.is_super or owning_interaction.sim is not self.sim:
					needs_derail = True
	...
	def get_uncarriable_objects_gen (self, posture_state = DEFAULT, allow_holster = DEFAULT, use_holster_compatibility = False):
		...
		for sim in self.required_sims():
			...
			if not sim.is_allowed_to_holster():
				...
				for (_, carry_posture, carry_object) in get_carried_objects_gen(sim):
					if self.allow_with_unholsterable_carries:
						...
						allow_holster = False
					break
					if not (carry_object.is_sim and not carry_object is not carry_target) and carry_object is not interaction_target:
			...
			for (carry_hand, carry_posture, carry_object) in get_carried_objects_gen(sim):
				...
				if (not current_interaction_constraint is None or not not forced_tentative_interaction_constraint == UNSET or carry_object is not carry_target) and carry_object is not interaction_target:
					for owning_interaction in list(carry_posture.owning_interactions):
						if not not use_holster_compatibility and not not carry_object.carryable_component.holster_compatibility(self.super_affordance):
							yield (owning_interaction, carry_posture)
						elif not not self.is_super and not not interactions.si_state.SIState.test_non_constraint_compatibility(self, owning_interaction, owning_interaction.target):
							yield (owning_interaction, carry_posture)
						else:
							...
							if not intersection.valid:
								...
								for sub_constraint in intersection:
									...
									if (not sub_constraint.posture_state_spec is None or body_target is not None) and not isinstance(body_target, PostureSpecVariable):
		...
		if self.progress_bar_enabled.bar_enabled and (self.display_name and self.sim.is_selectable) and self.sim.valid_for_distribution:
			if self.progress_bar_enabled.force_listen_statistic:
				...
				if percent is not None:
					if not self.progress_bar_enabled.remember_progress:
						if new_interaction:
							if percent < 1:
	...
	def is_affordance_available (cls, context = None):
		...
		if shift_held:
			...
			if cls.automation and paths.AUTOMATION_MODE:
				return True
		elif not cls.debug and not cls.cheat:
			return True
		return False
