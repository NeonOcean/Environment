...
class PrivacyLiability(Liability):
	...
	def __init__ (self, interaction, target = None, **kwargs):
		super().__init__(**kwargs)
		self._privacy = interaction.privacy.privacy_snippet(interaction)
		try:
...
class OwnsPostureLiability(ReplaceableLiability):
	...
	def should_transfer (self, continuation):
		if continuation.involves_carry or self._posture_ref is not None and isinstance(self._posture_ref(), CarryPosture):
			return False
		return True
					...
					if var_map_carry_target is None:
						...
						if hand is not None:
							...
							if temp_carry_posture.is_two_handed_carry:
								...
								break
				forced_tentative_interaction_constraint = None if carry_target is not None and carry_target.is_sim and posture_state is not None else current_interaction_constraint
