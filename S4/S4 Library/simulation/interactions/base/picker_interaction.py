...
class SimPickerMixin:
    ...
    @flexmethod
    def _get_valid_sim_choices_gen(cls, inst, target, context, test_function=None, **kwargs):
        inst_or_cls = inst if inst is not None else cls
...
class SimPickerInteraction(SimPickerMixin, PickerSingleChoiceSuperInteraction):
    ...
    def _run_interaction_gen(self, timeline):
        choices = tuple(self._get_valid_sim_choices_gen(self.target, self.context))
...
class ObjectInInventoryPickerMixin:
    ...
    def _setup_dialog(self, dialog, **kwargs):
        if self.suppress_duplicate_definitions:
            ...
            for obj in subject.inventory_component:
                self._definition_id_stack_counts[obj.definition.id] += obj.stack_count()
        super()._setup_dialog(dialog, **kwargs)
    ...
    @flexmethod
    def get_stack_count(cls, inst, obj):
        if cls.suppress_duplicate_definitions:
            return inst._definition_id_stack_counts[obj.definition.id]
...
class PurchasePickerInteraction(PickerSuperInteraction):
    ...
    @classmethod
    def has_valid_choice(cls, target, context, **kwargs):
        return cls.purchase_list_option.has_choices(cls, target=target, context=context, sim=context.sim, **kwargs)

    def _setup_dialog(self, dialog, **kwargs):
        purchase_picker_data = PurchasePickerData()
        self.delivery_method(self, purchase_picker_data)
        self.purchase_list_option(self, purchase_picker_data)
