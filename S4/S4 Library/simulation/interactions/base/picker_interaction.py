...
from event_testing.resolver import DoubleSimAndObjectResolver, InteractionResolver, SingleObjectResolver, SingleSimResolver
...
from interactions.base.picker_strategy import LotPickerEnumerationStrategy, ObjectPickerEnumerationStrategy, SimPickerEnumerationStrategy
...
from objects.object_tests import ObjectCriteriaSingleObjectSpecificTest, ObjectCriteriaTest, ObjectTagFactory, ObjectTypeFactory
from objects.terrain import get_venue_instance_from_pick_location
from plex.plex_enums import PlexBuildingType
from sims4.localization import LocalizationHelperTuning, TunableLocalizedStringFactory, TunableLocalizedStringFactoryVariant
from sims4.random import pop_weighted
from sims4.tuning.instances import lock_instance_tunables
from sims4.tuning.tunable import AutoFactoryInit, HasTunableSingletonFactory, OptionalTunable, Tunable, TunableEnumEntry, TunableFactory, TunableList, TunableMapping, TunableRange, TunableReference, TunableSet, TunableThreshold, TunableTuple, TunableVariant
...
from ui.ui_dialog_picker import LotPickerRow, ObjectPickerRow, ObjectPickerTuningFlags, PurchasePickerRow, SimPickerRow, TunablePickerDialogVariant, logger
...
class PickerSuperInteractionMixin:
	INSTANCE_TUNABLES = {
		'picker_dialog': TunablePickerDialogVariant(description = '\n            The object picker dialog.\n            ', tuning_group = GroupNames.PICKERTUNING),
		'pie_menu_option': OptionalTunable(description = '\n            Whether use Pie Menu to show choices other than picker dialog.\n            ', tunable = _TunablePieMenuOptionTuple(), disabled_name = 'use_picker', enabled_name = 'use_pie_menu', tuning_group = GroupNames.PICKERTUNING),
...
class SimPickerMixin:
	INSTANCE_TUNABLES = {
		...
		'target_continuation': TunableContinuation(description = '\n            If specified, a continuation to push on the target sim when a picker\n            selection has been made.\n            ', locked_args = {
			'actor': ParticipantType.TargetSim
		}, tuning_group = GroupNames.PICKERTUNING),
	...
	def _get_valid_sim_choices_gen (cls, inst, target, context, test_function = None, min_required = 1, **kwargs):
		...
		if (not (not inst_or_cls.sim_filter_household_override is not None or not participant is not None or inst_or_cls.sim_filter is not None) or not pre_filtered_sim_infos is not None or inst_or_cls.create_sim_if_no_valid_choices) and number_required_sims_not_found > 0:
...
class SimPickerInteraction(SimPickerMixin, PickerSingleChoiceSuperInteraction):
	INSTANCE_TUNABLES = {
		'picker_dialog': TunablePickerDialogVariant(description = 'The object picker dialog.', available_picker_flags = ObjectPickerTuningFlags.SIM, tuning_group = GroupNames.PICKERTUNING),
	...
	def has_valid_choice (cls, target, context, **kwargs):
		...
		for _ in cls._get_valid_sim_choices_gen(target, context, **kwargs):
			...
			if choice_count >= cls.picker_dialog.min_selectable:
				if choice_count >= cls.picker_dialog.min_choices:
					return True
		return False
...
class LotPickerMixin:
	INSTANCE_TUNABLES = {
		'default_inclusion': TunableVariant(description = '\n            This defines which venue types are valid for this picker.\n            ', include_all = TunableTuple(description = '\n                This will allow all venue types to be valid, except those blacklisted.\n                ', include_all_by_default = Tunable(bool, True), exclude_venues = TunableList(tunable = TunableReference(manager = services.get_instance_manager(sims4.resources.Types.VENUE), tuning_group = GroupNames.VENUES, pack_safe = True), display_name = 'Blacklist Items'), exclude_lists = TunableList(TunableVenueListReference(), display_name = 'Blacklist Lists'), locked_args = {
			'include_all_by_default': True
		}), exclude_all = TunableTuple(description = '\n                This will prevent all venue types from being valid, except those whitelisted.\n                ', include_all_by_default = Tunable(bool, False), include_venues = TunableList(tunable = TunableReference(manager = services.get_instance_manager(sims4.resources.Types.VENUE), tuning_group = GroupNames.VENUES, pack_safe = True), display_name = 'Whitelist Items'), include_lists = TunableList(TunableVenueListReference(), display_name = 'Whitelist Lists'), locked_args = {
			'include_all_by_default': False
		}), default = 'include_all', tuning_group = GroupNames.PICKERTUNING),
		'building_types_excluded': TunableSet(description = '\n            A set of building types to exclude for this map view picker. This\n            allows us to do things like exclude apartments or penthouses.\n            ', tunable = TunableEnumEntry(description = '\n                The Plex Building Type we want to exclude. Default is standard\n                lots. Fully Contained Plexes are apartments, and Penthouses are\n                regular lots that sit on top of apartment buildings and have a\n                common area.\n                ', tunable_type = PlexBuildingType, default = PlexBuildingType.FULLY_CONTAINED_PLEX, invalid_enums = (PlexBuildingType.INVALID,)), tuning_group = GroupNames.PICKERTUNING),
...
class MapViewPickerInteraction(LotPickerMixin, PickerSuperInteraction):
	INSTANCE_TUNABLES = {
		'picker_dialog': TunablePickerDialogVariant(description = '\n            The object picker dialog.\n            ', available_picker_flags = ObjectPickerTuningFlags.MAP_VIEW, tuning_group = GroupNames.PICKERTUNING, dialog_locked_args = {
			'text_cancel': None,
			'text_ok': None,
			'title': None,
			'text': None,
		...
		if inventory_subject is not None:
			if inventory_subject.inventory_component is not None:
				...
				for obj in inventory_subject.inventory_component:
					...
					if not not inst_or_cls.suppress_duplicate_objects is not None and key in used_object_keys:
						continue
...
class ObjectInSlotPickerMixin:
	INSTANCE_TUNABLES = {
		'subject_with_slots': TunableEnumEntry(description = '\n            Subject on which the slots exist.\n            ', tunable_type = ParticipantType, default = ParticipantType.Object, tuning_group = GroupNames.PICKERTUNING),
...
class TunableObjectTaggedObjectFilterTestSet(event_testing.tests.TestListLoadingMixin):
	...
	def __init__ (self, **kwargs):
		super().__init__(tunable = TunableVariant(object_criteria_test = ObjectCriteriaTest.TunableFactory(locked_args = {
			'subject_specific_tests': ObjectCriteriaSingleObjectSpecificTest(ObjectCriteriaTest.TARGET_OBJECTS, ParticipantType.Object),
			'tooltip': None,
			'identity_test': None,
			'test_events': ()
...
class AutonomousObjectTaggedPickerInteraction(AutonomousObjectPickerInteraction):
	...
	def _get_objects_internal_gen (cls, inst, target, context, **kwargs):
		...
		for obj in object_manager.get_objects_with_filter_gen(inst_or_cls.whitelist_filter):
			if not not sim_level != obj.level and not not inst_or_cls.allow_different_level:
				continue
			...
			if inst_or_cls.radius is not None:
				...
				if delta.magnitude() > inst_or_cls.radius:
					continue
			elif not not handle_outside and not obj.is_in_sim_inventory(sim) and obj.is_outside:
...
class InventoryItems(HasTunableSingletonFactory, AutoFactoryInit):
	FACTORY_TUNABLES = {
		...
		'object_requirement': OptionalTunable(description = '\n            The requirements the object must meet to be added to the picker. If\n            this is disabled, all inventory items will be displayed.\n            ', tunable = TunableVariant(description = '\n                The requirements for being added to the picker.\n                ', purchasable = TunableTuple(description = '\n                    Only show items that are in the Purchasable Objects list for\n                    the targets inventory component.\n                    ', locked_args = {
			'requirement_type': OBJ_REQUIREMENT_PURCHASABLE
		}), state_threshold = TunableTuple(description = '\n                    Only show items that meet this state threshold.\n                    ', threshold = TunableThreshold(description = '\n                        The state threshold the object must meet.\n                        ', value = TunableReference(manager = services.get_instance_manager(sims4.resources.Types.OBJECT_STATE), class_restrictions = 'ObjectStateValue')), locked_args = {
			'requirement_type': OBJ_REQUIREMENT_STATE_THRESHOLD
		}))),
