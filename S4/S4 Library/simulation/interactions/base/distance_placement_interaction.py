...
from interactions.constraints import TunableCircle, Nowhere, ANYWHERE
...
class DistancePlacementMixin:
    ...
    def setup_final_transforms(self, start_location, **fgl_kwargs):
        loc_b = start_location.duplicate()
    ...
    def _entered_pipeline(self):
        if self.carry_target is not None:
            ...
            if pick is not None and pick.pick_type == PickType.PICK_POOL_SURFACE:
                loc_a = sims4.math.Location(sims4.math.Transform(pick.location), pick.routing_surface)
