...
from interactions.constraints import ANYWHERE, Nowhere, TunableCircle, WaterDepthIntervalConstraint, WaterDepthIntervals
from interactions.social.social_super_interaction import SocialSuperInteraction
from interactions.utils.routing import FollowPath, PlanRoute
...
from sims4.tuning.tunable import OptionalTunable, Tunable, TunableAngle, TunableInterval, TunableRange
	...
	def _entered_pipeline (self):
		if self.carry_target is not None:
			...
			if pick is not None and pick.routing_surface.type == SurfaceType.SURFACETYPE_POOL:
				...
				fgl_kwargs.update({
									  'search_flags': fgl_flags,
									  'restrictions': (sims4.geometry.RelativeFacingRange(self.sim.position, 0),)
								  })
				...
				if self._starting_location is not None:
					...
					fgl_kwargs.update({
										  'max_distance': self.placement_distance.upper_bound,
										  'min_distance': self.placement_distance.lower_bound,
										  'restrictions': (sims4.geometry.RelativeFacingRange(loc_a.transform.translation, 0),),
										  'search_flags': fgl_flags
									  })
					self.setup_final_transforms(loc_a, **fgl_kwargs)
...
class FetchObjectSocialSuperInteraction(DistancePlacementMixin, SocialSuperInteraction):
	MAX_FETCH_RADIUS = 3
	INSTANCE_TUNABLES = {
		'fetch_constraint': TunableCircle(description = '\n            The circle constraint for other Sims in the social group to route\n            near the placement location.\n            ', radius = MAX_FETCH_RADIUS, tuning_group = GroupNames.CONSTRAINTS),
