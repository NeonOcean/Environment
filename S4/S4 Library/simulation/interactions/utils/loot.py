...
from bucks.bucks_loot_op import AwardPerkLoot, BucksLoot
...
from drama_scheduler.drama_node_ops import CancelScheduledDramaNodeLoot, ScheduleDramaNodeLoot
...
from interactions.utils.loot_ops import AddTraitLootOp, AwardWhimBucksLootOp, BreakThroughLootOperation, DestroyObjectsFromInventoryLootOp, DialogLootOp, DiscoverClueLootOp, DoNothingLootOp, ExtinguishNearbyFireLootOp, FireDeactivateSprinklerLootOp, FireLootOp, HouseholdFundsInterestLootOp, IncrementCommunityChallengeCount, LifeExtensionLootOp, LockDoor, NewCrimeLootOp, RefreshWhimsLootOp, RemoveNotebookEntry, RemoveTraitLootOp, ResetAspiration, SetPrimaryAspirationTrack, SlotObjects, StateChangeLootOp, SummonNPC, TravelToTargetSim, UnlockDoor, UnlockHiddenAspirationTrack, UnlockLootOp
...
from narrative.narrative_loot_ops import NarrativeGroupProgression, NarrativeLootOp
...
from objects.components.name_component import NameResetLootOp, SetNameFromObjectRelationship, TransferNameLootOp
...
from objects.components.stored_object_info_component import RemoveObjectInfoLootOp, StoreObjectInfoLootOp
from objects.components.stored_sim_info_component import RemoveSimInfoLootOp, StoreSimInfoLootOp, TransferStoredSimInfo
...
from relationships.relationship_knowledge_ops import KnowOtherSimCareerOp, KnowOtherSimMajorOp, KnowOtherSimTraitOp, KnowOtherSimsStat
from relationships.relationship_lock_change import UnlockRelationshipBitLock
from relics.relic_loot import AddRelicCombo
from restaurants.restaurant_ops import ClaimRestaurantSeat, ClaimRestaurantTable, ReleaseRestaurantTable, RestaurantExpediteGroupOrder
...
from sims.university.university_loot_ops import ApplyForScholarshipLoot, GetScholarshipStatusLoot, ScholarshipActionLoot, ShowHighChanceScholarshipsLoot, ShowScholarshipDynamicSignLoot, UniversityCourseGradeNotification, UniversityLootOp
...
from sims4.tuning.tunable import AutoFactoryInit, HasTunableReference, HasTunableSingletonFactory, Tunable, TunableList, TunableReference, TunableTuple, TunableVariant
...
from statistics.statistic_ops import DynamicSkillLootOp, DynamicVariantSkillLootOp, NormalizeStatisticsOp, SkillEffectivenessLoot, StatisticOperation, TunableStatisticChange
from topics.tunable import TopicUpdate
from tunable_multiplier import TunableMultiplier
from weather.weather_loot_ops import WeatherSetOverrideForecastLootOp, WeatherSetSeasonLootOp, WeatherStartEventLootOp
...
class LootActionVariant(TunableVariant):
	...
	def __init__ (self, *args, statistic_pack_safe = False, **kwargs):
		...
		}), dynamic_buff_loot = DynamicBuffLootOp.TunableFactory(), object_rewards = ObjectRewardsOperation.TunableFactory(), reward = RewardOperation.TunableFactory(), transfer_ownership = TransferOwnershipLootOp.TunableFactory(), create_object = ObjectCreationOp.TunableFactory(), create_puddles = CreatePuddlesLootOp.TunableFactory(target_participant_type_options = {
			'description': '\n                    The participant of the interaction whom the puddle\n                    should be placed near.\n                    ',
			'default_participant': ParticipantType.Object
		}), create_situation = CreateSituationLootOp.TunableFactory(), life_extension = LifeExtensionLootOp.TunableFactory(), notification_and_dialog = DialogLootOp.TunableFactory(), state_change = StateChangeLootOp.TunableFactory(), trait_add = AddTraitLootOp.TunableFactory(), trait_remove = RemoveTraitLootOp.TunableFactory(), know_other_sims_trait = KnowOtherSimTraitOp.TunableFactory(target_participant_type_options = {
			'description': '\n                    The Sim or Sims whose information the subject Sim is learning.\n                    ',
			'default_participant': ParticipantType.TargetSim
		}), know_other_sims_career = KnowOtherSimCareerOp.TunableFactory(target_participant_type_options = {
			'description': '\n                    The Sim or Sims whose information the subject Sim is learning.\n                    ',
			'default_participant': ParticipantType.TargetSim
		}), know_other_sims_statistics = KnowOtherSimsStat.TunableFactory(target_participant_type_options = {
			'description': '\n                    The Sim or Sims whose information the subject Sim is learning.\n                    ',
			'default_participant': ParticipantType.TargetSim
		}), know_other_sims_major = KnowOtherSimMajorOp.TunableFactory(target_participant_type_options = {
			'description': '\n                    The Sim or Sims whose information the subject Sim is learning.\n                    ',
			'default_participant': ParticipantType.TargetSim
		}), object_relationship = ObjectRelationshipLootOp.TunableFactory(target_participant_type_options = {
			'description': '\n                    The object whose relationship to modify.\n                    ',
			'default_participant': ParticipantType.Object
		}), interest_income = HouseholdFundsInterestLootOp.TunableFactory(), career_level = CareerLevelOp.TunableFactory(), career_loot = CareerLootOp.TunableFactory(career_options = {
			'pack_safe': True
		}), fire = FireLootOp.TunableFactory(), unlock_item = UnlockLootOp.TunableFactory(), fire_deactivate_sprinkler = FireDeactivateSprinklerLootOp.TunableFactory(), fire_clean_scorch = FloorFeatureRemoveOp.TunableFactory(), extinguish_nearby_fire = ExtinguishNearbyFireLootOp.TunableFactory(), award_whim_bucks = AwardWhimBucksLootOp.TunableFactory(), discover_clue = DiscoverClueLootOp.TunableFactory(), new_crime = NewCrimeLootOp.TunableFactory(), create_notebook_entry = NotebookEntryLootOp.TunableFactory(), breakthrough_moment = BreakThroughLootOperation.TunableFactory(), destroy_objects_from_inventory = DestroyObjectsFromInventoryLootOp.TunableFactory(), bucks_loot = BucksLoot.TunableFactory(), award_perk = AwardPerkLoot.TunableFactory(), refresh_whims = RefreshWhimsLootOp.TunableFactory(), remove_notebook_entry = RemoveNotebookEntry.TunableFactory(), lock_door = LockDoor.TunableFactory(), unlock_door = UnlockDoor.TunableFactory(), reaction = ReactionLootOp.TunableFactory(),
						 greeting = GreetingLootOp.TunableFactory(), event = ProcessEventOp.TunableFactory(), schedule_drama_node = ScheduleDramaNodeLoot.TunableFactory(), cancel_scheduled_drama_node = CancelScheduledDramaNodeLoot.TunableFactory(), set_club_gathering_vibe = SetClubGatheringVibe.TunableFactory(), summon_npc = SummonNPC.TunableFactory(), travel_to_target_sim = TravelToTargetSim.TunableFactory(), increment_community_challenge_count = IncrementCommunityChallengeCount.TunableFactory(), unlock_hidden_aspiration_track = UnlockHiddenAspirationTrack.TunableFactory(), set_primary_aspiration_track = SetPrimaryAspirationTrack.TunableFactory(), claim_table = ClaimRestaurantTable.TunableFactory(), claim_seat = ClaimRestaurantSeat.TunableFactory(), release_table = ReleaseRestaurantTable.TunableFactory(), restaurant_expedite_order = RestaurantExpediteGroupOrder.TunableFactory(), business_modify_customer_flow = ModifyCustomerFlow.TunableFactory(),
						 butler_state_change = ButlerSituationStateChange.TunableFactory(), slot_objects = SlotObjects.TunableFactory(), looping_loot_ops = LoopingLootOp.TunableFactory(), give_sickness = GiveSicknessLootOp.TunableFactory(), remove_sickness = RemoveSicknessLootOp.TunableFactory(), apply_tags_to_object = ApplyTagsToObject.TunableFactory(), make_pet_missing = MakePetMissing.TunableFactory(), name_reset = NameResetLootOp.TunableFactory(), post_missing_pet_alert = PostMissingPetAlert.TunableFactory(), vfx = PlayVisualEffectLootOp.TunableFactory(), add_relic_combo = AddRelicCombo.TunableFactory(), oneshot_broadcaster = BroadcasterOneShotLootOp.TunableFactory(), store_object_info = StoreObjectInfoLootOp.TunableFactory(), remove_object_info = RemoveObjectInfoLootOp.TunableFactory(), store_sim_info = StoreSimInfoLootOp.TunableFactory(), remove_stored_sim_info = RemoveSimInfoLootOp.TunableFactory(),
						 weather_set_override_forecast = WeatherSetOverrideForecastLootOp.TunableFactory(locked_args = {
							 'subject': ParticipantType.Actor
						 }), weather_set_season = WeatherSetSeasonLootOp.TunableFactory(locked_args = {
				'subject': ParticipantType.Actor
			}), weather_start_event = WeatherStartEventLootOp.TunableFactory(locked_args = {
				'subject': ParticipantType.Actor
			}), hidden_inventory_transfer = HiddenInventoryTransferLoot.TunableFactory(), transfer_painting_state = TransferPaintingStateLoot.TunableFactory(), squad_loot = SquadLootOp.TunableFactory(), stored_sim_info_transfer = TransferStoredSimInfo.TunableFactory(), custom_tooltip_transfer = TransferCustomTooltip.TunableFactory(), transfer_name_loot = TransferNameLootOp.TunableFactory(), reset_aspiration = ResetAspiration.TunableFactory(), narrative = NarrativeLootOp.TunableFactory(), narrative_progression = NarrativeGroupProgression.TunableFactory(), scheduled_delivery = ScheduledDeliveryLoot.TunableReference(), motherplant_battle_change = MotherplantBattleSituationStateChange.TunableFactory(), global_policy_add_progress = GlobalPolicyAddProgress.TunableFactory(locked_args = {
				'text': None
			}), festival_contest_get_reward = FestivalContestAwardWinners.TunableFactory(), set_name_from_object_relationship = SetNameFromObjectRelationship.TunableFactory(), create_plant = CreatePlantAtLocationLootOperation.TunableFactory(), set_favorite = SetFavoriteLootOp.TunableFactory(), roommate_ops = RoommateLootOp.TunableFactory(), university_course_grade_notification = UniversityCourseGradeNotification.TunableFactory(), university_loot = UniversityLootOp.TunableFactory(), organization_membership_loot = OrganizationMembershipLoot.TunableFactory(), scholarship_show_high_chance_loot = ShowHighChanceScholarshipsLoot.TunableFactory(), scholarship_apply_loot = ApplyForScholarshipLoot.TunableFactory(), scholarship_get_status_loot = GetScholarshipStatusLoot.TunableFactory(), scholarship_action_loot = ScholarshipActionLoot.TunableFactory(), scholarship_show_info_sign = ShowScholarshipDynamicSignLoot.TunableFactory(),
						 apply_loot_to_hidden_inventory_items = ApplyLootToHiddenInventoryItemsLoot.TunableFactory(), **kwargs)
