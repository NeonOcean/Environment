...
from sims4.tuning.tunable import AutoFactoryInit, HasTunableReference, HasTunableSingletonFactory, OptionalTunable, Tunable, TunableEnumEntry, TunableEnumFlags, TunableEnumSet, TunableFactory, TunableInterval, TunableList, TunablePackSafeReference, TunableRange, TunableReference, TunableSet, TunableThreshold, TunableTuple, TunableVariant, TunableWorldDescription
...
class StatisticFilterTerm(InvertibleFilterTerm):
	...
	FACTORY_TUNABLES = {
		'statistic': Statistic.TunableReference(description = '\n            The statistic the range applies to.\n            '),
		...
		'score_spread_override': OptionalTunable(
			...
			tunable = TunableInterval(tunable_type = float, default_lower = 0.0, default_upper = 1.0, minimum = 0)),
...
class SkillFilterTerm(StatisticFilterTerm):
	...
	FACTORY_TUNABLES = {
		'skill': Skill.TunableReference(description = '\n            The skill the range applies to.\n            '),
...
class RankedStatisticFilterTerm(StatisticFilterTerm):
	...
	FACTORY_TUNABLES = {
		'ranked_statistic': RankedStatistic.TunablePackSafeReference(description = '\n            The ranked statistic the range applies to.\n            '),
...
class CareerFilterTerm(InvertibleFilterTerm):
	...
	FACTORY_TUNABLES = {
		'career': TunableVariant(description = '\n            Define the career that this filter term applies to.\n            ', from_explicit_type = _CareerTypeExplicit.TunableFactory(), from_category = _CareerTypeFromCategory.TunableFactory(), default = 'from_explicit_type'),
		'career_location': TunableVariant(description = '\n            Define how important career location is for this filter. Leave None\n            if location is not important.\n            ', on_active_lot = _CareerLocationFilterActiveLot.TunableFactory(), default = None),
		'career_schedule': TunableVariant(description = '\n            Define how scheduling affects this filter. Leave None if schedule is\n            not important.\n            ', available_now = _CareerScheduleFilterAvailableNow.TunableFactory(), default = None),
...
class CasTagsFilterTerm(BaseFilterTerm):
	...
	def conform_sim_creator_to_filter_term (self, *, sim_creator, **kwargs):
		...
		sim_creator.tag_set.extend(cas_tags)
		return FilterResult.TRUE
...
class LivesOnStreetFilterTerm(DemographicsFilterTermMixin, InvertibleFilterTerm):
	...
	FACTORY_TUNABLES = {
		'street': OptionalTunable(description = '\n            The street the Sim must live on.\n            ', tunable = TunableWorldDescription(), disabled_name = 'current_street', enabled_name = 'specific_street'),
...
class LivesInCurrentRegion(InvertibleFilterTerm):
	...
	def calculate_score (self, sim_info, **kwargs):
		current_region_id = get_region_description_id_from_zone_id(services.current_zone_id())
		home_region_id = sim_info.household.get_home_region_id()
		score = 1 if current_region_id == home_region_id else 0
		return FilterResult(score = self.invert_score_if_necessary(score), sim_info = sim_info)

	def conform_sim_creator_to_filter_term (self, **kwargs):
		if not self.invert_score:
			return FilterResult('Unable to create a sim in who lives in the current region.', score = 0)
		return FilterResult.TRUE
...
class TunableSimFilter(HasTunableReference, metaclass = TunedInstanceMetaclass, manager = services.get_instance_manager(sims4.resources.Types.SIM_FILTER)):
	...
	INSTANCE_TUNABLES = {
		...
		'specify_cas_randomization_mode': OptionalTunable(description = '\n            If enabled, the CAS sim randomization mode to use for this filter.\n            \n            If disabled, the default client randomization mode logic will be used.\n            ', disabled_name = 'USE_CLIENT_DEFAULT', tunable = TunableEnumEntry(tunable_type = RandomizationMode, default = RandomizationMode.SELECTIVE_RANDOMIZATION)),
		'specify_cas_filter_flags': OptionalTunable(description = '\n            If enabled, the flags to use for this filter.\n            \n            If disabled, the default filter flags will be used.\n            ', disabled_name = 'USE_CLIENT_DEFAULT', tunable = TunableEnumFlags(enum_type = OutfitFilterFlag, default = OutfitFilterFlag.NONE))
	}
	...
	@flexmethod
	def _repurpose_sim_info (cls, inst, sim_info, **kwargs):
		inst_or_cls = inst if inst is not None else cls
		conformable_terms = []
		for term in inst_or_cls.get_filter_terms():
	...
	def create_sim_info (cls, inst, zone_id, blacklist_sim_ids = (), **kwargs):
		...
		if inst_or_cls.repurpose_terms is not None:
			...
			for unimportant_result in services.sim_filter_service().submit_filter(repurpose_terms, None, blacklist_sim_ids = blacklist_sim_ids, allow_yielding = False, gsi_source_fn = inst_or_cls.get_sim_filter_gsi_name, **kwargs):
				result = inst_or_cls._repurpose_sim_info(unimportant_result.sim_info, **kwargs)
				if result:
		...
		if sim_creator.breed_name_key == 0:
			...
			if breed_tag is not None:
				sim_creator.tag_set.append(breed_tag)
		...
		if inst_or_cls.specify_cas_filter_flags is not None:
			sim_creator.filter_flags = inst_or_cls.specify_cas_filter_flags
