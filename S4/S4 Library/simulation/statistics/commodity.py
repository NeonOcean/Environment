...
class CommodityState:
    __slots__ = ('_value', '_buff', '_icon', '_fill_level', '_buff_add_threshold', '_data_description', '_data_description_overrides', '_fill_color', '_background_color', '_tooltip_icon_list', '_tooltip_icon_list_overrides', 'loot_list_on_enter', 'apply_loot_on_load')

    def __init__(self, value=0, buff=None, icon=None, fill_level=None, buff_add_threshold=None, data_description=None, data_description_overrides=None, fill_color=None, background_color=None, tooltip_icon_list=None, tooltip_icon_list_overrides=None, loot_list_on_enter=None, apply_loot_on_load=False):
        self._value = value
        self._buff = buff
        self._icon = icon
        ...
        self._data_description_overrides = data_description
...
class Commodity(HasTunableReference, TunedContinuousStatistic, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(sims4.resources.Types.STATISTIC)):
    ...
    def fixup_on_sim_instantiated(self):
        ...
        if self.time_passage_fixup_type() == CommodityTimePassageFixupType.FIXUP_USING_TIME_ELAPSED:
            ...
            if time_sim_was_saved is not None and not sim.is_locked(self):
                self.decay_enabled = True
                self.update_commodity_to_time(time_sim_was_saved)
                self.decay_enabled = False
