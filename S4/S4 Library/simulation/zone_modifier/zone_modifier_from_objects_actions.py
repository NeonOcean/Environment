...
class ZoneModifierFromObjectsAction(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'multiplier': Tunable(description='\n            The multiplier to apply to the object count.\n            ', tunable_type=float, default=0), 'participant': TunableEnumEntry(description="\n            The participant of which we're running the action.\n            ", tunable_type=ParticipantTypeLot, default=ParticipantTypeLot.Lot)}
    ...
    def _get_participant_stat_tracker(self, stat):
        ...
        if target is None:
            logger.error('Failed to get participant {}, when trying to get tracker for Zone Modifier Statistic Change', self.participant)
            return
        else:
            tracker = target.get_tracker(stat)
            if tracker is None:
                logger.error('Failed to get tracker of statistic {} from {}, when trying to get tracker for Zone Modifier Statistic Change', stat, target)
                return
        return tracker
