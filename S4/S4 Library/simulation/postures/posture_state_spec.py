            ...
            if allow_surface:
                if slot_manifest:
                    for slot_manifest_entry in slot_manifest:
                        ...
                        slotted_object = None
                    if slotted_object is not None:
                        if slotted_object.definition is not slotted_object:
                        ...
                        if all(posture_type.mobile for (posture_type, _) in posture_types):
                            ...
                            for child in surface_target.children:
                                ...
                                if surface_target is not None:
                                    ...
                                    posture_manifest = posture_manifest.apply_actor_map(get_surface)
                                    slot_manifest = slot_manifest.apply_actor_map(get_surface)
            elif not isinstance(body_target, (AnimationParticipant, PostureSpecVariable, str)):
                if surface_target is None or isinstance(surface_target, (AnimationParticipant, PostureSpecVariable, str)):
                    surface_target = None
