...
class TransitionSequenceController:
    ...
    def intended_location(self, sim):
        if self.running and not self.canceled and not (not self.interaction.is_finishing and not self.is_derailed(sim)):
    ...
    def _wait_for_violators(self, timeline, blocked_sims):
        ...
        def wait_for_violators(timeline):
            ...
            while True:
                ...
                if not (self._blocked_sis and self._blocked_sis) and not any(blocked_sim.routing_component.get_stand_slot_reservation_violators() for blocked_sim in blocked_sims):
                    cancel_functions()
                    return
                    ...
                    if not interaction.is_cancel_aop:
                        if not self.involves_specific_surface_or_body_target(final_constraint):
                            if not final_constraint.supports_mobile_posture(body_aspect.posture_type):
                                posture_object = None
                                if sim.in_pool and not self.has_geometry_outside_pool(final_constraint, sim):
    ...
    def has_geometry_outside_pool(self, constraint, sim):
        ...
        for sub_constraint in constraint:
            ...
            for sub_surface in sub_routing_surfaces:
                if not sub_surface is None and not sub_surface.type == routing.SurfaceType.SURFACETYPE_POOL:
    ...
    def _target_location_changed(self, obj, *args, **kwargs):
        if self.interaction.carry_target is obj or self.interaction.target is obj:
    ...
    def _get_transitions_for_sim(self, timeline, sim, target_sim=None, target_path_spec=None, ignore_inertial=False, ignore_combinables=False):
        ...
        try:
            ...
            if not success and (path_spec.failed_path_type != PathType.MIDDLE_LEFT and (path_spec.failed_path_type != PathType.MIDDLE_RIGHT and all_destinations - self._tried_destinations[sim])) and path_spec.destination_spec is not None:
        ...
        for constraint in final_constraint:
            ...
            if posture_state_spec is None:
                ...
                if not new_posture_manifest:
                    ...
                    new_posture_state_spec = PostureStateSpec(FrozenPostureManifest(new_posture_manifest), posture_state_spec.slot_manifest, posture_state_spec.body_target)
    ...
    def _can_change_outfits_from_posture(self, posture):
        return posture.posture_type == StandSuperInteraction.STAND_POSTURE_TYPE or posture.posture_type == SIM_SWIM_POSTURE_TYPE
    ...
    def create_transition(self, create_posture_state_func, si, current_transition, var_map, participant_type, sim, *additional_sims):
        ...
        if self._can_change_outfits_from_posture(sim.posture_state.body):
            if not any(self._can_change_outfits_from_posture(remaining_transition.body_posture) for remaining_transition in remaining_transitions):
        ...
        if last_nonmobile_posture_with_entry_change:
            entry_change = last_nonmobile_posture_with_entry_change.post_route_clothing_change(last_nonmobile_posture_with_entry_change, self.interaction, do_spin=True, sim_info=sim.sim_info)
        ...
        if not self._processed_on_route_change:
            if entry_change is None:
                if self._can_change_outfits_from_posture(sim.posture_state.body) or self._can_change_outfits_from_posture(posture_state.body):
        ...
        if clothing_change is None:
            if outdoor_streetwear_change is not None:
                if self._can_change_outfits_from_posture(sim.posture_state.body) or self._can_change_outfits_from_posture(posture_state.body):
                    clothing_change = build_critical_section(sim.sim_info.get_change_outfit_element(outdoor_streetwear_change, do_spin=True, interaction=self.interaction), flush_all_animations)
        ...
        if not (source_interaction is None or not source_interaction.visible):
        ...
        if clothing_change is not None:
            ...
            elif self._can_change_outfits_from_posture(posture_state.body):
    ...
    def _create_transition_interaction(self, timeline, sim, destination_spec, create_posture_state_func, target, participant_type, target_si=None, linked_sim=None):
        ...
        for (si, var_map) in transition_spec.transition_interactions(sim):
            ...
            if executed_path:
            ...
            elif current_spec == destination_spec and (transition_spec.path is None and not has_pre_route_change) and not transition_spec.portal_obj:
                run_transition_gen = None
            else:
                ...
                def run_transition_gen(timeline):
                    ...
                    if not result_transition and not ((not self.is_derailed(sim) or self._derailed[sim] == DerailReason.TRANSITION_FAILED) and not self.canceled and not self._shortest_path_success[sim]):
        ...
        def do_transition_multi_exit(timeline):
            ...
            if linked_path_spec is not None:
                ...
                for linked_sim_destination_posture_type in valid_exit_posture_types:
                    ...
                    if not source_posture_type.mobile and linked_sim_destination_posture_type.mobile:
                        body_target = None
                    else:
    ...
    def _get_putdown_transition_info(self, sim, actor_transitions, current_state, next_state):
        ...
        if not (current_body_posture_target is None or not current_body_posture_target.is_sim) and next_body_posture_target is not None and next_body_posture_target.is_sim:
            ...
            if preferred_carrying_sim is not carrying_sim:
                ...
                animation_work = None
        elif not (current_body_posture_target is not None and current_body_posture_target.is_sim and (next_body_posture_target is None or not next_body_posture_target.is_sim)):
        ...
        if master is sim:
            ...
            for slave_data in slave_datas:
                ...
                for si in incompatible_sis:
                    if not si.is_finishing and si.user_cancelable:
        ...
        if next_carry_target.is_sim:
            ...
            if carry_target_transitions:
                ...
                if carry_target_next_body_target is not sim:
                    return (None, None, None, None, None)
        if (not current_state.carry_target is None or not next_carry_target is not None or next_body_posture_target is not None) and next_body_posture_target.is_sim and next_body_posture_target is not current_body_posture_target:
            return (None, None, None, None, None)
        ...
        if self._transition_canceled:
            ...
            if not no_work:
                ...
                if work is not None:
                    ...
                    if not multi_sim_exit_sim and self._interaction.is_putdown and actor_transitions is not None:
