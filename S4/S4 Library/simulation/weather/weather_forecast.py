...
class WeatherForecast(HasTunableReference, metaclass=HashedTunedInstanceMetaclass, manager=services.get_instance_manager(Types.WEATHER_FORECAST)):
    ...
    def is_forecast_supported(cls, options, snow_safe, rain_safe):
        ...
        if prescribed_weather_type is None:
            return True
        if prescribed_weather_type.rain and (not rain_safe or options[PrecipitationType.RAIN] == WeatherOption.WEATHER_DISABLED):
            return False
        if prescribed_weather_type.snow and (not snow_safe or options[PrecipitationType.SNOW] == WeatherOption.WEATHER_DISABLED):
            return False
        if prescribed_weather_type.storm and (not rain_safe or options[PrecipitationType.RAIN] == WeatherOption.DISABLE_STORMS):
            return False
        elif prescribed_weather_type.blizzard and (not snow_safe or options[PrecipitationType.SNOW] == WeatherOption.DISABLE_STORMS):
            return False
        return True
