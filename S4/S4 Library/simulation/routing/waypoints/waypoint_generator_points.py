...
class _WaypointGeneratorObjectPoints(_WaypointGeneratorBase):
    ...
    def get_waypoint_constraints_gen(self, sim, waypoint_count):
        ...
        object_waypoint = Circle(self._target.position, self.waypoint_constraint_radius, routing_surface=self._target.routing_surface, los_reference_point=self._target.position)
        ...
        for position in random_uniform_points_in_compound_polygon(polygon, num=waypoint_count):
            object_waypoint_constraints.append(Circle(position, self.waypoint_constraint_radius, routing_surface=self._start_constraint.routing_surface, los_reference_point=self._target.position))
            object_waypoint_constraints.append(object_waypoint)
