...
from sims4.tuning.tunable import TunableList, TunableRange, Tunable, TunableTuple, OptionalTunable
...
class FormationTypeFollow(FormationTypeBase):
    ...
    def route_length_minimum(self):
        return self._route_length_minimum
    ...
    def find_good_location_for_slave(self, master_location):
        ...
        if fgl_tuning.slave_should_face_master:
            ...
            fgl_kwargs.update({'raytest_radius': self.RAYTRACE_RADIUS, 'raytest_start_offset': self.RAYTRACE_HEIGHT, 'raytest_end_offset': self.RAYTRACE_HEIGHT, 'ignored_object_ids': {self.master.id, self.slave.id}, 'max_distance': self._offset.magnitude(), 'min_distance': self._offset.magnitude()})
    ...
    def should_slave_for_path(self, path):
        ...
        if path_length >= self._route_length_minimum or final_dist_sq >= self._route_length_minimum*self._route_length_minimum:
            return True
        return False
    ...
    def update_slave_position(self, master_transform, master_orientation, routing_surface, distribute=True, path=None, canceled=False):
        ...
        if path is not None and path.length() < self._route_length_minimum and (self._slave.position - slave_position).magnitude_squared() < self._route_length_minimum*self._route_length_minimum:
            return
        if distribute:
