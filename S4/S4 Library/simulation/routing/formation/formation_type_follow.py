...
class FormationTypeFollow(FormationTypeBase):
    ...
    def find_good_location_for_slave(self, master_location):
        ...
        if new_position is None or new_orientation is None:
            if fgl_tuning.fallback_fgl:
                logger.info('No good location found for {} after slaved in a routing formation headed to {}. Trying master_location instead.', self.slave, starting_location)
                fgl_context = FindGoodLocationContext(master_location, object_id=self.slave.id, object_footprints=(footprint,) if footprint is not None else None, search_flags=fgl_flags, restrictions=restrictions, min_distance=fgl_tuning.fallback_fgl.lower_bound, max_distance=fgl_tuning.fallback_fgl.upper_bound, **fgl_kwargs)
                (new_position, new_orientation) = placement.find_good_location(fgl_context)
        if new_position is None or new_orientation is None:
