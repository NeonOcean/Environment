...
from placement import FGLSearchFlagsDefault, FGLSearchFlagsDefaultForSim, FGLSearchFlag, FindGoodLocationContext
...
from sims4.math import Vector2, Vector3, MAX_INT32, Quaternion, Location
from sims4.tuning.geometric import TunableVector2
from sims4.tuning.instances import HashedTunedInstanceMetaclass
from sims4.tuning.tunable import TunableList, HasTunableReference, TunableReference, TunableRange, TunableMapping, Tunable
...
class RoutingFormation(HasTunableReference, metaclass=HashedTunedInstanceMetaclass, manager=services.snippet_manager()):
    ...
    def find_good_location_for_slave(self, master_location):
        ...
        if self.slave_should_face_master:
            ...
            fgl_kwargs = {'raytest_radius': RoutingFormation.RAYTRACE_RADIUS, 'raytest_start_offset': RoutingFormation.RAYTRACE_HEIGHT, 'raytest_end_offset': RoutingFormation.RAYTRACE_HEIGHT, 'ignored_object_ids': {self.master.id, self.slave.id}, 'max_distance': self._offset.magnitude(), 'min_distance': self._offset.magnitude()}
