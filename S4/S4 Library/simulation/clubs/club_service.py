...
from clubs import UnavailableClubCriteriaError, club_tuning
from clubs.club import Club, ClubOutfitSetting
from clubs.club_enums import ClubGatheringKeys, ClubGatheringStartSource, ClubHangoutSetting, ClubRuleEncouragementStatus
from clubs.club_telemetry import TELEMETRY_FIELD_CLUB_BUCKSAMOUNT, TELEMETRY_FIELD_CLUB_HANGOUTLOT, TELEMETRY_FIELD_CLUB_HANGOUTSETTING, TELEMETRY_FIELD_CLUB_HANGOUTVENUE, TELEMETRY_FIELD_CLUB_ID, TELEMETRY_FIELD_CLUB_NPCS, TELEMETRY_FIELD_CLUB_NUMRULES, TELEMETRY_FIELD_CLUB_TOTALCLUBS, TELEMETRY_FILED_CLUB_PCS, TELEMETRY_HOOK_CLUB_COUNT, TELEMETRY_HOOK_CLUB_CREATE, TELEMETRY_HOOK_CLUB_OVERVIEW, club_telemetry_writer
from clubs.club_tuning import ClubTunables
from distributor.ops import SendClubBuildingInfo, SendClubInfo, SendClubMembershipCriteriaValidation, SendClubValdiation, ShowClubInfoUI
...
from world.region import Region, get_region_instance_from_zone_id
...
class ClubService(Service):
	...
	def __init__ (self):
		...
		self.broadcaster_extra = type('Club_Rule_Broadcaster', (BroadcasterRequest, object), {
			'broadcaster_types': lambda *_, **__: [club_tuning.ClubTunables.CLUB_RULE_BROADCASTER],
	...
	def create_club (self, club_seed = None, seed_members = None, club_data = None, from_load = False, refresh_cache = True):
		...
		encouragement_commodity = type(encouragement_name, (StaticCommodity, object), {
			'ad_data': club_tuning.ClubTunables.CLUB_ENCOURAGEMENT_AD_DATA
		})
		discouragement_commodity = type(discouragement_name, (StaticCommodity, object), {
			'ad_data': club_tuning.ClubTunables.CLUB_DISCOURAGEMENT_AD_DATA
		})
		encouragement_buff = type(encouragement_name, (Buff, object), {
			'visible': False
		})
		discouragement_buff = type(discouragement_name, (Buff, object), {
			'visible': False
		})
		social_autonomy_mod = AutonomyModifier(relationship_score_multiplier_with_buff_on_target = {
			encouragement_buff: club_tuning.ClubTunables.CLUB_RELATIONSHIP_SCORE_MULTIPLIER
		})
		social_game_effect_mod = type('Club_Autonomy_Mod', (GameEffectModifiers, object), {
			'_game_effect_modifiers': [social_autonomy_mod]
		})
		social_encouragement_buff = type(social_encouragement_name, (Buff, object), {
			'visible': False,
			'game_effect_modifier': social_game_effect_mod
		})
		new_club = Club(club_id, name, icon, description, encouragement_commodity, discouragement_commodity, encouragement_buff, discouragement_buff, social_encouragement_buff, leader = leader, leader_id = leader_id, member_ids = member_ids, recent_member_ids = recent_member_ids, membership_criteria = membership_criteria, rules = club_rules, hangout_setting = hangout_setting, hangout_venue = hangout_venue, hangout_zone_id = hangout_zone_id, invite_only = invite_only, uniform_male_child = uniform_male_child, associated_color = associated_color, uniform_female_child = uniform_female_child, uniform_male_adult = uniform_male_adult, uniform_female_adult = uniform_female_adult, club_seed = club_seed, bucks_tracker_data = bucks_tracker_data, male_adult_mannequin = male_adult_mannequin, male_child_mannequin = male_child_mannequin, female_adult_mannequin = female_adult_mannequin, female_child_mannequin = female_child_mannequin, outfit_setting = outfit_setting,
