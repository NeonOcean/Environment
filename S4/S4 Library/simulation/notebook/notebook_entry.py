...
class NotebookEntryBait(NotebookEntry):
    ...
    def _get_entry_tooltip(self, entry_def):
        return EntryTooltip(HovertipStyle.HOVER_TIP_DEFAULT, {TooltipFields.recipe_name: LocalizationHelperTuning.get_object_name(entry_def), TooltipFields.recipe_description: LocalizationHelperTuning.get_object_description(entry_def), TooltipFields.rarity_text: self._get_entry_rarity_text(entry_def), TooltipFields.subtext: self._get_entry_size_text(entry_def)})
