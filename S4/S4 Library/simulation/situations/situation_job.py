...
class SituationJobReward(HasTunableSingletonFactory, AutoFactoryInit):
    FACTORY_TUNABLES = {'reward': Reward.TunableReference(allow_none=True), 'loot': LootActions.TunableReference(allow_none=True), 'loot_target': OptionalTunable(description='\n                If enabled then we will use the tuned situation job to pick a\n                random sim in the owning situation with that job to be the\n                target of the loot operation.\n                ', tunable=TunableReference(description='\n                    The situation job that will be used to find a sim in the\n                    owning situation to be the target sim.\n                    ', manager=services.get_instance_manager(sims4.resources.Types.SITUATION_JOB)))}
    ...
    def _apply_loot(self, sim, situation):
        ...
        self.loot.apply_to_resolver(resolver)
...
class SituationJob(HasDependentTunableReference, metaclass=HashedTunedInstanceMetaclass, manager=services.situation_job_manager()):
    ...
    def _verify_tuning_callback(cls):
        ...
        if tuned_tags and len(tuned_tags) != len(set(tuned_tags)):
    ...
    def get_score(cls, event, resolver, **kwargs):
        if event == event_testing.test_variants.TestEvent.InteractionComplete:
            ...
            for score_list in cls.interaction_scoring:
                if score_list.role_tags is not None:
                    if target is None:
                        continue
                    should_apply_score = False
                    for situation in services.get_zone_situation_manager().get_situations_sim_is_in(target):
                        role_tags = situation.get_role_tags_for_sim(target)
                        if score_list.role_tags.blacklist & role_tags:
                            should_apply_score = False
                            break
                        if score_list.role_tags.whitelist is None:
                            should_apply_score = True
                        elif score_list.role_tags.whitelist & situation.get_role_tags_for_sim(target):
                            should_apply_score = True
                            if not score_list.role_tags.blacklist:
                                break
                    if not should_apply_score:
                        pass
                    elif resolver(score_list.affordance_list):
                        return score_list.score
                if resolver(score_list.affordance_list):
                    return score_list.score
        elif event == event_testing.test_variants.TestEvent.ItemCrafted:
            for score_list in cls.crafted_object_scoring:
                if resolver(score_list.object_list):
                    return score_list.score
        return 0
    ...
    def can_sim_be_given_job(cls, sim_id, requesting_sim_info, gsi_source_fn=None):
        ...
        return services.sim_filter_service().does_sim_match_filter(sim_id, sim_filter=cls.filter, requesting_sim_info=requesting_sim_info, additional_filter_terms=cls.get_location_based_filter_terms(), gsi_source_fn=gsi_source_fn or get_sim_filter_gsi_name)
