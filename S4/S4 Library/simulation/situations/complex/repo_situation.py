...
from sims4.tuning.tunable import TunableTuple, TunableInterval, TunablePercent, TunableList, OptionalTunable
...
class _RepossessObjectState(CommonInteractionCompletedSituationState):
    ...
    def _on_interaction_of_interest_complete(self, resolver=None, **kwargs):
        debt_statistic = self.owner.get_debt_statistic()
        debt_statistic.add_value(-self.owner.current_object.depreciated_value)
        self.owner.clear_current_object()
        self._change_state(self.owner.leave_state())
...
class RepoSituation(SituationComplexCommon):
    ...
    def _cache_valid_objects(self):
        debt_statistic = self.get_debt_statistic()
        if debt_statistic is None:
            self._self_destruct()
        target_amount = debt_statistic.get_value()*self.repo_amount.target_amount
        unsorted = []
        for obj in services.object_manager().valid_objects():
            resolver = SingleObjectResolver(obj)
            if not obj.get_household_owner_id() == self.debtor_sim().household_id:
                continue
    ...
    def get_debt_statistic(self):
        host_sim_info = services.sim_info_manager().get(self._guest_list.host_sim_id)
        return host_sim_info.get_statistic(LoanTunables.DEBT_STATISTIC, add=False)
