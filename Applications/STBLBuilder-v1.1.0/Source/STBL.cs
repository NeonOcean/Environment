	...
	public static class STBL {
		...
		public static void Build (File file, string buildPath) {
			...
			foreach(KeyValuePair<string, string> language in Languages) {
				string languageFileName = string.Format(file.Name, language.Key);

				string sourceFileName = languageFileName + "." + STBLFileExtension;
				string sourceFilePath = Path.Combine(buildPath, sourceFileName);

				using(BinaryWriter writer = new BinaryWriter(new FileStream(sourceFilePath, FileMode.Create))) {
				...
				if(STBLBuilder.Entry.BuildSourceInfoFile) {
					string sourceInfoFilePath = sourceFilePath + "." + SourceInfoFileExtension;

					Tools.WriteXML(sourceInfoFilePath, new SourceInfo() {
						Name = languageFileName,
						Type = "220557DA",
						Group = file.Group,
						Instance = language.Value + file.Instance.Substring(2)
					});
				}
			}
		}
